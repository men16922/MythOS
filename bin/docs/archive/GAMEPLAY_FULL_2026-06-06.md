...
> 이 문서는 **엔터테인먼트/게임플레이** 관점의 권위 있는 설계다. 세계관/비전 archive는 `docs/archive/DRAFT.md`,
> 시스템/아키텍처/데이터 모델은 `docs/DESIGN.md`, 현재 상태/로드맵은 `docs/STATUS.md` / `docs/NEXT_PLAN.md`를 따른다.
...
6. **기억이 곧 세계.** 모든 선택은 기록되고, 세계는 그 기억으로 다시 태어난다. (`docs/archive/DRAFT.md` 철학 계승)
...
- **Run History / 기록 보관소:** 엔딩 또는 archive마다 세션 요약, 엔딩, 전투, 단서, 동료, 해금 결과를
  저장하고 플레이어 메뉴에서 다시 읽는다. 로그라이크의 run history처럼 “이번 접속이 어떤 이야기였는지”를
  캠페인 기억으로 보존한다.
- **Meta Progression:** run summary를 기반으로 새 아키타입, 시작 아이템, 시작 위치, 동료 후보, Codex 진실
  조각을 해금한다. 해금은 다음 루프 시작 선택지와 초기 state에 반영한다.

### 7.1 Story Bible / 게임북

시나리오는 두 층으로 관리한다.

- `scenario.json`: 런타임이 읽는 구조화 데이터. arcs, NPC agendas, endings, combat pool, system prompt를 포함한다.
- `story_bible`: 작가가 쓰는 세계관/설정/스토리/타임라인/이벤트/대사 조각. LLM GM이 필요할 때 참고하는
  게임북이다.

원칙:

- Story Bible 전체를 매 턴 프롬프트에 넣지 않는다.
- 현재 phase, location, flags, NPC 상태, 단서, 엔딩 압력에 맞는 snippet만 선택해 `NarrativeContext`에 넣는다.
- LLM은 snippet을 정사(canon) 제약과 영감으로 사용하되, 플레이어가 인게임 문서나 대사를 발견한 상황이
  아니면 원문을 그대로 덤프하지 않는다.
- 신규 샘플 시나리오 후보는 `세계 : 접속 - 유리성의 사서`다. 붕괴 중인 기억 도서관에서 “첫 번째 접속
  기록”을 찾는 3막 미스터리로 구성한다.

### 7.2 Save / Load

저장 원칙:

- 권위 상태는 계속 `LoopState`와 store에 둔다.
- active loop는 load 가능한 save로 표시한다.
- ended loop는 load 대상이 아니라 Run History 대상이다.
- 장면 시작/선택 후 autosave를 기본으로 하고, 필요 시 save slot metadata를 추가한다.
- 전투 중 저장은 `_combat` state를 포함하되 UI에서 “전투 중”으로 명확히 표시한다.
...
## 10.5 스탯, 속성 및 자율성 시스템 (RPG Stats & Autonomy)

이 파트는 플레이어가 관찰자에서 세계의 촉매로 진화해 나가는 RPG적 스탯과 자율성 메커니즘을 정의한다.

### 10.5.1 접속자 5대 핵심 스탯 (Core Stats: 1~10 Scale)
모든 스탯은 1에서 10까지의 척도를 가지며, AI GM은 이 수치를 기준으로 행동 판정의 성패와 묘사의 난이도를 결정한다.
...
| **관측 (Perception)** | 데이터 및 진실 포착 | 단서(Shard) 발견 확률 증가, 시스템 글리치 이면의 진실 감지. |

### 10.5.2 사이버-신화 속성 (Cyber-Mythic Attributes)
플레이어의 정체성을 특화하며 고유한 서사적 혜택이나 선택지를 제공한다.
- **`[ARK 링크]`**: 시스템 직접 동기화. 해킹 보너스 제공 및 관리망 감시 긴장도(Tension) 상승 위험 동반.
- **`[유령 신호]`**: 소거된 비접속자. 은밀한 이동에 가산점 제공, 일반 NPC 소통에 페널티.
- **`[인간찬가]`**: 비효율적 의지. 절망적 상황에서의 NPC 신뢰 획득율 상승.
- **`[시스템 글리치]`**: 인과적 모순 변수. 무작위 행운 및 긴장도 비례 돌발 파괴력 발휘.
- **`[기억의 직공]`**: 과거 기록 수집가. 장면 내 단서(Narrative Shard) 발견 가중치 부여.

### 10.5.3 진행도 기반 자율성 (Autonomy Progression)
플레이어의 자율성 레벨(Autonomy Level: 1~5)은 캠페인 도달도에 따라 서서히 상승하며, 레벨에 부합하지 않는 과격 행동 지시 시 AI GM이 캐릭터의 심리적 주저함을 반영해 '심리적 가드레일'을 묘사하도록 프롬프트에 동적 전달된다.

*   **LV 1 수동적 신호**: 기본 선택지 순응. 시스템 상식 밖의 위험 지시 거부.
*   **LV 2 동조된 관찰자**: 질문, 제한적 의문 제기 가능. NPC 협동 시작.
*   **LV 3 의식의 균열**: 시스템 명령에 대한 명시적 거부 표현 가능.
*   **LV 4 능동적 변수**: 위험한 해킹, 탈취, 공격 등 가드레일 대폭 완화.
*   **LV 5 해방된 촉매**: 세계 물리 법칙 우회, 시스템 코드 직접 재작성 및 각성.
