...
- Changed: Added `bin/docs/plans/2026-06-21-overnight-auto-agy-qa.md`: existing `make overnight*` remains the only operator flow; post-gate/critic and DONE-time triggers decide whether AGY browser QA is needed; Chrome DevTools→Playwright MCP direct actor, dedup ledger, STOP/notify semantics, status/report integration, command cleanup, fault matrix, and WS-A~F are specified.
- Verified: Plan read-back complete and `git diff --check -- bin/docs/plans/2026-06-21-overnight-auto-agy-qa.md` passed. Prior implemented baseline remains `make check` green (462 tests, skipped 2) and AGY-direct WS0 `PASS_CANDIDATE`.
...
- Status: Wired the companion-cutscene gallery into the frontend per the breadcrumb in `bin/docs/plans/2026-06-16-companion-affection-cutscenes.md` §P1-갤러리. Backend already exposes `memoryOverview.cutscene_gallery` (`cutscenes.cutscene_gallery()` → `serializers.memory_overview_to_dict` → `GET /api/v1`); this closes the FE read/render slice. `make check` green (462).
...
- Status: Wired the companion-affection gauge into the frontend per the breadcrumb in `bin/docs/plans/2026-06-16-companion-affection-cutscenes.md` §P0-게이지. Backend already exposes `snapshot.state.relationships`; this closes the FE read/render slice. `make check` green (462).
...
- Changed: ① NEW `resources/neo-seoul/directives/encounters.md`(`travel_header:`/`emergency_header:` file-meta + `## travel_template`/`## emergency_low_stability`/`## emergency_high_tension` 3블록). ② `scenario_directives.py`: `Encounters` 데이터클래스(travel_header/travel_template/emergency_header/2 emergency 템플릿) + `ScenarioDirectives.encounters: Encounters|None` + `_encounters_from_parsed`(빈 문서→None) + `load_scenario_directives`가 `encounters.md` 로드 + `__all__` export. ③ `scenario_context.py`: `DEFAULT_ENCOUNTERS` 앵커 상수 + 4 inline f-string note → `directives.encounters or DEFAULT_ENCOUNTERS` + `fill_placeholders` generic 주입으로 재작성(키워드 list/임계 분기는 동일 유지).
- Verified: `make check` EXIT=0 — ruff/eslint/mypy(116 files)/frontend build + **395 tests OK**(skipped 2, +5). `EncounterDirectiveParityTest` 5건: byte-parity(loaded==`DEFAULT_ENCOUNTERS`)·런타임 context travel directive 주입(action='지하로 이동한다')·emergency 양쪽 임계(stability20/tension80) 주입·glass-library None+기본값 폴백 banner 잔존(회귀0)·빈 문서→None.
- Blockers: 없음.
- Next: L/M/N(호감도 런타임 — feature, 아침 검수 필요) 등 잔여 `[auto:claude]`. Phase 4 prose 추출은 4c로 일단락(4a naming/4b stat-voice/4c encounter). 실제 최우선은 Neo-Seoul 사람 QA([manual]).
...
## 2026-06-21 (s) — overnight [auto:claude]: extract useCombatCinemaQueue hook from App.tsx
- Status: Behavior-preserving frontend god-component decomposition, slice 5. The combat-board cinema-queue driver moved out of App.tsx into a hook, matching the `useGameSocket`·`useTypewriter`·`useCombatBoard`·`useInGameEpiphany` pattern.
- Changed: NEW `src/mythos_ui/src/hooks/useCombatCinemaQueue.ts` — owns `cinemaContext`/`cinemaQueue` state + `pendingTransitionRef`/`prevCombatRef`/`dispatchedActionRef` + the ~120-line canvas-draw `useEffect` (diffs prev→next combat snapshots into per-blow CombatCinema overlays or board tweens) + the internal `playTerminalCombatSfx` helper, and exposes `onCinemaImpact` (interim board draw at the overlay impact frame) + `onCinemaFinish` (advance queue, run the deferred board animation). Takes `{finalizedSnapshot, selectedScenarioId, fallbackMode, activeTab, canvasRef, animatorRef, playSfx}`; returns the state + the two refs (`prevCombatRef` reset by `handleSimulateCombat`, `dispatchedActionRef` set by `handleCombatAction`) + the two callbacks. `App.tsx`: removed the cinema state/refs block, the `playTerminalCombatSfx` def, the driver `useEffect`, and inlined the render block's `onImpact`/`onFinish` closures (now `={onCinemaImpact}`/`={onCinemaFinish}`); dropped the now-unused `CombatState`/`CombatLogEntry`/`CombatCinemaContext` type imports. `animatorRef`/`canvasRef` stay in App (shared with `useCombatBoard`). App.tsx 1261→1102 lines. No logic change.
- Verified: `make check` green — ruff + eslint + mypy (124 files) + tsc/vite-build + **512 tests OK** (skipped 2); doc-budget + skills mirror checks pass. Pure extraction; no test-count change (FE has no unit-test runner; gate guarantees compile/type/lint only).
- Blockers: none. Post-commit AGY live-QA (auto-screened, §3.4.1) is the runner's job; not run in this iteration.
- Next: continue App.tsx/CombatCinema decomposition one slice per iteration (onboarding/session-lifecycle handlers `handleStartGame`/`handleResumeGame`/`handleSimulateCombat`, or the visual-status/image-resolve concern are next candidates).

## 2026-06-21 (r) — overnight [auto:claude]: extract useGameSocket hook from App.tsx
- Status: Behavior-preserving frontend god-component decomposition, slice 4. The WebSocket connect + auto-reconnect lifecycle moved out of App.tsx into a hook, matching the `useTypewriter`·`useCombatBoard`·`useInGameEpiphany` pattern.
- Changed: NEW `src/mythos_ui/src/hooks/useGameSocket.ts` — owns `websocketRef` + the 4 reconnect refs (`reconnectAttemptsRef`/`reconnectTimeoutRef`/`isExplicitCloseRef` + the `MAX_RECONNECT_ATTEMPTS` const) + `openSocket` (connect, parse frame → `onMessage`, exponential-backoff reconnect on unexpected close) + `closeSocket` (explicit-close teardown). Takes `{onMessage, logToConsole}`, returns `{websocketRef, openSocket, closeSocket}`. `App.tsx`: removed the 5 inline refs + the ~64-line `openSocket`; added a `handleSocketMessage` type-switch (token/snapshot/visual_status/error — unchanged) passed as `onMessage`; `handleLeaveSession`'s inline socket teardown (~13 lines) collapsed to `closeSocket()`; dropped the now-unused `getWebSocketUrl` import (moved into the hook). Hook call placed before `sendChoose` (which reads `websocketRef`) to satisfy `react-hooks/immutability`; added `websocketRef` to `sendChoose` deps. No logic change.
- Verified: `make check` green — ruff + eslint + mypy (124 files) + tsc/vite-build + **512 tests OK** (skipped 2); doc-budget + skills mirror checks pass. Pure extraction; no test-count change (FE has no unit-test runner; gate guarantees compile/type/lint only).
- Blockers: none. Post-commit AGY live-QA (auto-screened, §3.4.1) is the runner's job; not run in this iteration.
- Next: continue App.tsx/CombatCinema decomposition one slice per iteration (combat-cinema queue effect / canvas-draw `useEffect`, or onboarding/session-lifecycle handlers are next candidates).

## 2026-06-21 (q) — overnight [auto:claude]: extract useTypewriter hook from App.tsx
- Status: Behavior-preserving frontend god-component decomposition, slice 3. The typewriter narration concern moved out of App.tsx into a hook, matching the `useAudio`·`useCombatCinema`·`useInGameEpiphany`·`useCombatBoard` pattern.
- Changed: NEW `src/mythos_ui/src/hooks/useTypewriter.ts` — owns `displayedNarration`/`isStreaming` state + the 4 streaming refs (`narrationQueueRef`/`narrationTypedRef`/`streamDoneRef`/`pendingSnapshotRef`) + the typewriter `setInterval` loop + `startTyper` + a `resetStreamBuffers` helper; takes `onFinalizeSnapshot` (passed `setFinalizedSnapshot`) and returns the state/setters/refs/helpers. `App.tsx`: replaced the inline state+refs block, the `startTyper` callback, and the typewriter `useEffect` (~40 lines) with the hook call; the two inline buffer-reset sequences (in `beginStream` and `handleStartGame`) now call `resetStreamBuffers()`. `narrationTypedRef` is hook-internal (no longer destructured). No logic change.
- Verified: `make check` green — ruff + eslint + mypy (124 files) + tsc/vite-build + **512 tests OK** (skipped 2). Pure extraction; no test-count change (FE has no unit-test runner; gate guarantees compile/type/lint only).
- Blockers: none. Post-commit AGY live-QA (auto-screened, §3.4.1) is the runner's job; not run in this iteration.
- Next: continue App.tsx/CombatCinema decomposition one slice per iteration (WS reconnect refs/`openSocket`, or the combat-cinema queue effect are next candidates).
...
<!-- archived from PROGRESS_LOG 2026-06-30 (tidy-docs) -->
