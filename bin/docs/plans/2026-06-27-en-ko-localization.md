...
- `[x]` **S1 영어 생성(A)** — **완료(2026-06-28, make check green/538 tests)**: `prompts.py`에 `STORY_SYSTEM_PROMPT_EN`/`DEFAULT_SYSTEM_PROMPT_EN`/`JSON_CONTRACT_EN`/`OPENING_FIRST_SCENE_INSTRUCTION_EN` + 언어 분기 seam(`_story_system_prompt`/`_system_prompt`/`_json_contract`/`_opening_first_scene_instruction`, 전부 `context.language=="en"` 선택); `fallbacks.py` `DEFAULT_FALLBACK_BY_LANG`+`default_fallback()`(EN C-17 폴백 산문); `director._fallback_payload` 언어별 폴백+연결어(`_FALLBACK_CONNECTIVES`); `smoke.py --language` + `make narrative-smoke-fallback-en`; `tests/test_language_plumbing.py` +9(EN 프롬프트/계약/폴백 영어 산출 락). 행동 보존: **default 여전히 `ko`** — default 플립은 EN 저작물(S2)과 함께(지금 KO 라이브-QA 병행 중이라 조기 플립 시 골든패스 깨짐). **잔여(S1→이월)**: API/UI 언어 선택 + 루프/플레이어 state 영속.
- `[/]` **S2 golden-path 저작물 EN(B)** — **S2a 완료(2026-06-28, make check green/543 tests)**: directives 로더 `*.{lang}.md` 분기(`_directive_path`, `<name>.<lang>.md`→`<name>.md` graceful fallback, cache 키에 lang, companions dedupe) + `scenario_context` 코드룰 EN(`LANGUAGE_RULE_EN`/`CINEMATIC_CLARITY_RULE_EN` + `_language_rule`/`_cinematic_clarity_rule` 선택자) + neo-seoul EN directive 5종(`opening`/`fallback`/`naming`/`stat_voices`/`encounters`.en.md, 구조 parity: flags `met_se_rin`/`refused_se_rin`·`start_combat: patrol_ambush`·placeholders·choice suffix/intent 동일). 테스트 `test_language_plumbing.py` +5(EN directive 로드·구조 parity·placeholder·미현지화 시나리오 KO 폴백). **S2b 진행(2026-06-28, make check green/547)**: 리스크 회피를 위해 **additive overlay** 채택(코어 `load_scenario`/ContextVar 미변경 — §7.2 ContextVar 파괴적 추출은 §7.7 최고위험이고 KO 라이브-QA 병행 중이라 보류). `load_scenario_i18n(scenario_id, lang)` = `resources/<scenario>/i18n/<lang>.json` 가산 오버레이(부재 시 `{}` → KO 소스 그대로, 무위험). neo-seoul `i18n/en.json`(brief + session_intro:title/body/objective/cinematic_shots×3 + main_arcs6 + side_arcs6 + endings4 + npc_agendas5, 전부 영어). 주입부 분기: `SCENARIO_BRIEF` + `_opening_continuity_notes`(오버레이 데이터 + `_opening_continuity_lines_en` 영어 프레이밍) + `_scenario_structure_notes`(`_localized_named_items` index-merge, per-item KO 폴백·npc 키 현지화). 테스트 +8. **S2b 잔여**: 기타 노트빌더(`_route_director_notes`/`_route_junction_notes` 등) 한국어 프레이밍·bible 스니펫·glass-library 오버레이. (출력 언어는 `LANGUAGE_RULE_EN`이 이미 영어 강제 — 잔여는 모델에 주는 *지시문*이 이중언어일 뿐, 산출은 영어.) 완료기준(전체): 오프닝~첫 전투~종료 경로가 영어 저작물로 통함.
- `[/]` **S3 UI i18n — 모든 플레이어향 화면 완료(2026-06-28, make check green/549)**: `src/i18n/`(strings.{ko,en}.ts ~210키, 키-parity `Record<StringKey,string>`로 tsc 강제 / `lang.ts` resolveInitialLang+useLang / `LangProvider.tsx`), `main.tsx` 루트 래핑, HeaderBar KO↔EN 토글(localStorage 영속). 치환 완료: **App(상태/탭알림/깨달음배너)·Onboarding·Header·TabNav·ChoicePanel·BootIntro(크롬)·GameAside(route/status/log)·StoryPanel(전투결과/전술범례/타일/목표/히스토리/엔딩 + 이중언어 스탯-모놀로그 파서)·CombatControls·CombatLog·CombatRoster·RouteNarrative·CharacterPanel·SaveHistoryPanel·CharacterTabPanel·CutsceneGallery·CodexPanel·SkillTreePanel·ProgressDashboard**. **영어 opt-in만**(`?lang=en`/localStorage; navigator 자동감지는 플립과 함께). **S3 잔여(의도적 보류)**: DevConsolePanel(dev 전용 탭), GameAside `_map` 레거시 분기(제거 예정), BootIntro/IntroPanel의 scenario `ui_copy` 폴백(=API가 현지화 scenario 서빙해야 — S1잔여 "API 언어 선택"과 연동), HeaderBar 브랜드명(고유). 완료기준 충족: 온보딩~플레이~종료 UI 영어, `make frontend-build` green.
