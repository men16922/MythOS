...
> ▶ NEXT SESSION: **Prod = `mythos-api-00090-ww4` (2026-09-08, HEAD `d4a87da` deployed, nothing pending; 0 WARNING+).** Live: the opening-cinematic/build-offer order fix and status-effect stacking (2026-09-06; `docs/plans/2026-09-06-status-effect-stacking.md`)** — owner feel verdict on caps 3/2/2 + pip legibility is `[manual]`, and **boss stack resistance shipped 2026-09-07** (`BOSS_STACK_CAP = 2`, agent call under the continue directive — DECISIONS; IX solo ×3 0.82→0.68, tick-kills 33%→15%; one constant to reverse), **live on `00090`**. Migration `008` is applied on prod (2026-09-07, owner-run `make db-migrate-prod`). Everything else open in NEXT_PLAN is owner-gated (§3 verdict, stack/boss-cap feel verdict — both play on `00090`) or `[blocked]` (P2 bench port needs `labs/wsl2-vllm-baseline/`; adapter rows need an engine). `[auto]` backlog is **0** (the 16-seed batch fully closed 2026-09-07, last item the closed-plan-doc move `1aea723`, done supervised since `bin/` is outside the claude lane) — seed only from a new owner decision, do not manufacture. `main` ahead of `origin/main` by 84 — owner `git push`. **Do not**: build a stream-stall watchdog; add keyword inflections on principle; decay stacks per turn (only with expiry — design §Stack decay); put `experiments/`/`scripts/<other>` paths in `[auto:claude]` seeds.
