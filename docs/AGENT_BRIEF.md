...
> ▶ NEXT SESSION: **2026-07-12 session #14 (combat VISUAL overhaul V1-V6; `make check` 1041 green; origin+3; UNDEPLOYED on top of live `mythos-api-00055-46x`)**: owner probe verdict on "시각적으로 별로임" = **ALL FOUR** areas + two live requests → shipped code-side: **V1** camera drag-pan + `getIsoConfig` centering bugfix (10×7 arenas clipped right-edge enemies) · **V2** node-tinted backdrop (biome from encounter id; art hook `combat/backdrops/<biome>.png`) + floor de-noise + vignette/rim · **V3** cell-true AoE (exact chebyshev tiles, diamond rings, corner-chevron range, red out-of-range) · **V4** status badge chips (dark disc + color rim ABOVE name — was icon-on-name mush) · **V5** cinema impact slashes + speed-lines + big damage numbers + grenade item-art center card · **V6** full skill glyphs (제어/강화 were bare letters) + consumable thumbnails. Design `docs/plans/2026-07-12-combat-visual-overhaul.md`; before/after `outputs/vis-diag/01..32`. CONTINUE HERE: (1) **status stacking** `[auto:claude]` (owner-decided: duration ACCUMULATES on reapply + multi-status concurrency guarantees — NEXT_PLAN combat section, gameplay-qa verify); (2) `! git push` → owner `make deploy` → **owner feel pass** (live-QA A-1/A-3/A-4; owner already ticked A-0 #3/#5/#6 locally); (3) verdict gates **agy art seeds** (backdrop plates ×4 · flat badge glyphs ×7 · floor tile · cover_full prop); (4) triage: **한 시스템 침투 ◆4 > max FOCUS 3 = uncastable** · 린위에 victory-lineup 누락 · loot pill raw ids. Late adds: grenade item-card cut-in for 0-dmg throws + blast ×2.7 + skip grace + tutorial 다음 button + local BGM off (`7882d3d`/`c7165f9`). Traps: `make deploy` billable+PROD (owner `!`); fallback = static combat by design; don't edit the repo while an overnight runner is live.
...
1. **Combat visual overhaul V1-V6 (2026-07-12, UNDEPLOYED)**: camera pan + centering fix, node backdrops, cell-true AoE, badge chips, cinema impact, icon coverage — deploy then owner feel pass gates the agy art seeds (backdrop plates, flat badge glyphs, floor tile). Design: `docs/plans/2026-07-12-combat-visual-overhaul.md`.
