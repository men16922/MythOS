...
> ▶ NEXT SESSION: **Implement the variant-routed opening — S1→S2/S3 first** (user-directed 2026-07-06; design `docs/plans/2026-07-06-variant-routed-opening.md`). Live evidence: B2 variants evaporate at turn 1 (layer-0 anchor `opening_escape` + connect chapter-gate + canon pull the Se-rin rail back). S1 anchor `variants` resolution · S2 `player_goal_variants` · S3 se_rin flag clamp — all `[auto:claude]`, `make check` green, loop-1 byte-identical; S4 directive/copy = human tone verdict (drafts in plan §4). Context: overnight lanes drained 07-06 (opening art ×6 codex `d661f44` + per-variant boot intro `ca5c835`+hold-screen, icons ×10 + SFX ×4 `b095446`, live-QA ×2 PASS). Local test stack may still run (`make api-cloud`, port 8000). Humans: deploy new bundle + sign-off (`docs/test/neo_seoul_live_qa.md` refreshed) · G2 twist tone review · voice-id pinning · `git push` · ally-writeback triage. Known trap: nested agy-in-agy live-QA hangs — run direct. Rollback = `MODEL=gemini-2.5-flash` env.
