...
- P3 Web UI 디커플링 slice 1·2·3·4(B) 완료: `src/mythos_api/`가 `RuntimeSessionService`를 FastAPI `/api/v1` REST(`auth/connect`, `loops/begin|active|choose`, `combat/begin|action`, `health`) + WebSocket 토큰 스트리밍(`loops/stream`) + 자산 presigned URL 변환(`assets/resolve`)으로 노출하고, 경량 PoC 클라이언트(`static/{index.html,app.js}`)를 `/`에 서빙. Streamlit 무변경 추가형, optional `web` extra, `python -m mythos_api`로 실행. 테스트는 `tests/test_api.py`(in-memory TestClient, 17 tests). 분산 visual worker는 `visual_worker.py`로 기존 구현됨.
- 그다음(선택, 별도 트랙): P3 slice 4 옵션 A 풀 Next.js/Vite SPA + PixiJS Canvas 전술 보드. 프론트 결정/스펙은 `docs/plans/2026-06-03-frontend-slice4.md`, 백엔드 설계는 `docs/plans/2026-06-03-web-ui-decoupling.md`.
