...
> ▶ NEXT SESSION: **Continue Landscape Combat — seed+run `[auto:claude]` LC5 then LC6** (`docs/plans/2026-07-08-design-system.md` "Landscape Combat"). Context: mobile-UX day landed P1.5 mobile-first + Design-System Phase 0/1 (DS0 tokens · DS1a `Surface` · DS1b one `Popover`) + Landscape Combat LC0-4 over 4 overnight runs (sonnet pinned in `overnight-settings.json`); `make check` **948** green, ahead **57**. **Emulator-verified the CLOUD build myself** (`make api-cloud`, Chrome-devtools @844×390): landscape combat page is now **1.00× (no page scroll ✅)** but header+tab-nav eat ~215px and the action bar is scroll-only → **LC5 = hide tab-nav + shrink header in landscape combat; LC6 = reorder the right column actions-first (Attack/Defend/Skills+targets before roster/Save/STATUS)**, then re-verify on the emulator (AGY can't — it runs a desktop viewport). Arm via `MAX_ITER=4 make overnight`. **Human-only/blocked:** real-device pass · **DS2/DS3** `[blocked]` (review the DS1a `Surface` API + pick density defaults) · **T5c** `[blocked]` (iso legibility @390px) · S4/T4 카피 톤 · **`git push` (ahead 57)** · tear down `api-cloud` (pid was 5031). Finding: concise mode only −10% in combat (board/roster/controls can't collapse) → combat needs landscape, not density. Known trap: nested agy-in-agy live-QA hangs — run direct.
