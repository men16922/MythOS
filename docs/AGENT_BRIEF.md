...
> ▶ NEXT SESSION: **Continue the DS2 `Surface` panel sweep — run `[auto:claude]` DS2-c (combat cluster) next** (`docs/plans/2026-07-08-ds2-sample-migration.md` §8). Context: this session landed **LC5/LC6** (Landscape Combat track COMPLETE, LC0-6, emulator-verified: page 1.00×/no-scroll, board+actions co-visible) **and unblocked+started DS2** — the design-system panel migration. Owner signed off the DS2 review packet (G1 default `size-2`=16px · G3 bake glow into `.surface-surface` · G2 accept 6→8px radius); **DS2-api + DS2-a (StatusPanel) + DS2-b (aside cluster: OperationMap ×2, Save/RunHistory)** are done, `Surface` now forwards `...rest`/`id`, emulator computed-style parity confirmed (only delta = intended +2px radius). `make check` **953** green; **origin pushed to `889f713`** (07-08, user pushed the whole batch — remaining gap is Cloud Run **deploy**, not push). **Remaining DS2 = `[auto:claude]`, one cluster per slice**: DS2-c combat · DS2-d intro/onboarding · DS2-e modals · DS2-f dashboards · then the 2 deferred `<details className="panel">` chips (need a Surface `as` prop) + bespoke classes (~25 `.panel` sites left). Pattern is proven-mechanical: swap `.panel`→`<Surface variant="surface">`, delete dead container CSS, `make check` green + emulator/AGY parity. **Human-only/blocked:** **DS3** density defaults (unblocks the density system) · **T5c** iso @390px judgment · real-device pass · S4/T4 카피 톤 + 2회차 feel run · **Cloud Run redeploy** (push done `889f713`; live rev `00025-856` is stale) · tear down `api-cloud` (pid was 5031, Vertex-billed). **`PROGRESS_LOG.md` at 119/120 — run `/tidy-docs` before the next increments.** Traps: AGY can't see landscape (desktop viewport) → landscape/mobile needs the emulator; reload the cloud build with cache-bypass to pick up a fresh bundle.
