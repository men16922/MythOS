...
> ▶ NEXT SESSION: **P1-a companion cutscene appearance is complete locally (2026-07-03, `make check` 665 + smoke-local + Playwright E2E green):** CBT consolidation was pushed and deployed as Cloud Run rev `mythos-api-00005-ljj`; final P1-a push/redeploy is pending. Eligible companion cutscenes now appear once per loop on the next safe transit scene with localized full-render script lock, curated image/title, `scene_type=cutscene`, and save/resume-safe active/seen state; opening, anchor, side-reveal, and combat turns defer without consuming. **NEXT = finish push/redeploy, then human live play-QA** (`docs/test/neo_seoul_live_qa.md` 🔴: IX boss fires · P1-a Se-rin cutscene · side-scene variation · 30–60min feel). Closed beta remains live on Neon + Vertex + GCS with invite gate, tester cap, and scale-to-zero. Other human follow-ups: Discord/r/playtesters distribution, monitor sign-ups, K9 natural-ending screenshot. App.tsx slice-14 remains `[blocked]`; Korean-on-resume is persisted legacy prose only. Runbook `docs/cloud/DEPLOY.md` §10.**
