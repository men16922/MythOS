...
> ▶ NEXT SESSION: **S4 — variant-routed opening content** (the only piece left of `docs/plans/2026-07-06-variant-routed-opening.md`; S1-S3 mechanisms LANDED overnight 07-07, `087f891`/`cab08a5`/`49f5205`, check 901 green independently re-verified). S4 = author the 6 anchor `variants` data blocks + chapter-gate `player_goal_variants` copy (drafts in plan §4) + extend directive windows 0→3 — **human tone verdict required** (register: screenplay action-line). Until S4 lands the variant still converges to the Se-rin rail in-game (mechanism ships no data). Also open: `[blocked]` shot 03 ×6 (codex image quota — remove tag when quota resets, shot 02 landed `5568afd`) · humans: deploy+sign-off (`docs/test/neo_seoul_live_qa.md`) · G2 twist tone · voice-id pinning · `git push` (ahead 15) · ally-writeback triage. Local stack: `make api-cloud` port 8000. Known trap: nested agy-in-agy live-QA hangs — run direct. Rollback = `MODEL=gemini-2.5-flash` env.
