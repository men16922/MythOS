...
> ▶ NEXT SESSION: **DEPLOYED rev `mythos-api-00032-zrl` (2026-07-09, `make check` 961, smoke ok; deploy via `make deploy` = .env PROJECT_ID — ambient gcloud project drifted once → stray service in `claude-study-501117`, deleted). Latest live: session #4 — Se-rin no longer leaks into loop-2+ variant openings (bible `opening_variant` gate + `solo_opening` suppression of carried met/party companions + solo re-entry directive; Vertex-verified turn 0-2 clean). Also live: mobile "대본 최우선" (#3: CHARACTER chip dropped on coarse, ObjectiveStrip one-line collapsible, narration 46vh cap lifted, anchor-image grid blowout `minmax(0,1fr)`, tab overflow) + #3b setup `<select>`/action-row overflow (all screens h-overflow-free @390px; market/boss/ending not swept). Remaining: `! git push` (user, hard-block; git ahead of origin) + real-device pass + ally-writeback triage (root of the Se-rin issue).** Prior session (all live): **Se-rin-flash reopened bug FIXED** (early `loop_meta` WS frame gives the client the opening variant before the slow generation, killing the 8s-timeout default-then-swap — **owner-confirmed working live**) · **mobile UX**: header **hidden mid-play** behind a floating ⋯ menu (language/COMPACT/BGM/leave), narration-first (scene image → 16:9 banner, inline CHARACTER → collapsed chip), **tab rename** to one clear i18n noun each (이야기/도감/인물/스킬, KO+EN). Earlier this day also live: DS0–DS3 · LC0-6 · S4 · P1.5 · opening prose. `make check` **956** green; WS order `loop_meta→token→snapshot` test-locked; mobile emulator-verified @390px (header display:none + floating ⋯ + banner + chip + 42px tab row). **Human-only now:** **`! git push`** (git ahead of origin) · **real-device phone pass** (mobile is emulator-only — touch/notch/`100dvh` URL-bar) · optional S4/T4 카피 톤 · T5c iso @390px. Traps: E2E uses port 8080 (adminer conflict → use 8099); fresh browser defaults EN; chrome-devtools window has a ~500px min width — use the `emulate` tool for a true 390px viewport; `make api` (Ollama) is free for layout checks, `make api-cloud` (Vertex) is billable — stop it (`make api-stop`) when done.
