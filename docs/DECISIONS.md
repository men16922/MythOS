...
Reason/impact: the 2026-08-09 hold existed so the owner's §3 ending/overall verdict would be read against the same build the arm was played on. Lifting it means the banked `prod-people-help-20260808` arm is **no longer comparable on the changed axes** (chips, portraits, opening art, refusal branch) to any arm played on the new build — so the next promotion sample must be a fresh arm on the new revision, and the 08-08 arm's feel verdict, if it is still given, is a verdict on the old build. The judge-noise decision (median-N / pin the judge / drop the numeric gate) is unaffected and still open.

## 2026-08-30 — Self-hosting inference is deferred as a scale question, and MythOS becomes a research workload instead

Decision: production narrative stays on the pay-per-token cloud API. The self-hosting question is not closed by preference but **deferred against a stated threshold**, and the engineering effort is redirected to making MythOS a measurable serving-research workload (`docs/plans/2026-08-30-mythos-as-serving-research-workload.md`).

Reason/impact: the inference-engineering text puts disaggregation's entry at 100M–1B tokens/day and states that a latency-sensitive application with light, unpredictable traffic leaning on scale-to-zero for cost reasons is signalling it is *not* ready for dedicated infrastructure. Neo-Seoul measures ~15k output tokens per loop, so the threshold is thousands of loops/day — orders of magnitude away. Impact: no GPU procurement, no disaggregation, no orchestration layer (measured at −32.6% throughput and +200% TTFT at this concurrency), and draft-model speculation stays out on measured evidence (−42.2% KV budget, no win at any concurrency). What is kept is the **measurement** that decides when this reverses: per-turn token usage (already logged) plus prefix-cache hit rate. Reversal criterion is explicit — recompute the crossover with real prices once a week of token data exists. Detail: `docs/reference/2026-08-30-self-hosted-inference-and-mythos-as-research-platform.md`.

## 2026-08-30 — Sampler settings are translated per engine, and context length is not a setting at all

Decision: one engine-neutral `SamplerSpec` is rendered per engine by `mythos_narrative/engine_options.py` (`MYTHOS_LLM_ENGINE`), and anything an engine cannot carry is dropped **explicitly, where the reason is written down**. `OLLAMA_NUM_CTX` is deleted rather than fixed.

Reason/impact: `OllamaJSONProvider` had been sending six options as `extra_body={"options": {...}}` — the *native* Ollama shape — while the runtime talks to the OpenAI-compatible endpoint, which discards it silently. Measured: `num_predict=5` delivered that way returned 691 chars, byte-identical to uncapped; as `max_tokens`, 0. So `num_ctx`/`num_predict`/`repeat_penalty`/`repeat_last_n`/`top_p`/`top_k` had **never once applied locally**, including the repetition tuning that exists to fight a tracked open risk. Impact: `repeat_penalty` (multiplicative) is translated to `frequency_penalty` (additive) — a translation, not an equivalence, and labelled so; `top_k` is undeliverable on this endpoint and is dropped rather than posted; local degradation went 44% → 0%. Context length is **not a per-request parameter** on an OpenAI-compatible endpoint — Ollama reads it from the Modelfile — so keeping a config field for it would leave a knob that looks live and does nothing, which is the exact failure being fixed; pick a `*-64k` model variant instead. The `vllm`/`llamacpp`/`mlx` rows are marked **unverified** and must be measured with `make experiment ARGS=option-matrix` before use. **Any local repetition judgement made before this date was made against a model that never received the setting**; production (Vertex Gemini) is a separate path and unaffected.

## 2026-08-30 — Experiment reports must declare their limits, and prompt traces are source-equivalent

Decision: `experiments/harness.py` **refuses to render a report whose `limits` are empty** and leaves no directory behind; every run records controls, variables, git commit/dirty state and a SHA-256 of the experiment script; more than one variable prints a warning that the run cannot attribute its own result; findings carry `measured`/`reproduced`/`indicative` and nothing else. Reports are never overwritten — one stamped directory per run. Prompt traces are treated as source-equivalent and are **not publishable**.

Reason/impact: the method is inherited from a study whose central finding was that the same technique swings from +199% to −54% with workload and load, so an unqualified number invites exactly the comparison that produces the wrong answer; a convention would not have held, and this is the same discipline `narrative_judge.py` already applies by hashing the rubric. It paid immediately: two stamped `workload-profile` reports showed a plausible fix (raising `num_ctx`) changing nothing, which a single overwritten report would have hidden. Traces carry full prompts — scenario canon and narrative prose — so they fall under the same non-publication rule as `resources/<scenario>/story_bible`; report `raw/data.json` holds aggregates only (verified: zero Hangul), so reports are shareable while traces are not. Experiments stay outside `make check` (live engines, minutes, non-deterministic) and are barred from the unattended loop.
...
(`worktrees.sh` LINK_DIRS=""). 운영/검증 권위 `docs/engineering/mythos/AGENTIC.md`, 블루프린트 `bin/docs/archive/AI_TEAM_BLUEPRINT.md`.
...
Reason: 실측 A/B(`bin/docs/plans/2026-07-04-gemini-2.5-vs-3.5-eval.md`) — 3.5는 감각 묘사 밀도가 확실히 오르지만 파싱/지연은 동급이고 비용이 4.7×($1.0–1.3/루프). 턴 입력 ~10k 중 고정분이 절반이라 캐싱이 입력비를 크게 깎는다.

Impact: CBT 기본값은 2.5 유지. 캐싱 구현+실측 후 3.5 전환 여부 결정. 벤치/토큰 프로브는 `scratch/gemini_model_bench.py`·`gemini_token_probe.py`로 재실행 가능.

### 서사 모델 gemini-3.5-flash 채택 (2026-07-04 밤)

Decision: CBT 서사 모델을 gemini-2.5-flash → **gemini-3.5-flash**로 전환한다 (Cloud Run rev `mythos-api-00018-pj9`, env `MODEL` 한 줄; 로컬도 동일). 컨텍스트 캐싱은 비용 레버로 **기각**한다.
...
Reason: research-backed density scale (Material comfortable/compact −4dp) + game-HUD inspector pattern (Into the Breach / XCOM: one fixed inspector, decision-critical stats only), per `bin/docs/plans/2026-07-08-design-system.md` Phase 3. 48px min tap target is preserved because density only shrinks container padding, never buttons.
