...
백로그의 ~75%가 **사람 플레이 체감 QA**(`docs/neo_seoul_live_qa.md` 전부)·**콘텐츠/Story-Bible 저작**·
**밸런스 튜닝**·**LLM 프롬프트-feel 튜닝**으로, 무인 에이전트가 검증할 수 없다.

→ 따라서 이 LOOP는 **hygiene / regression / refactor / codemod / deterministic-bugfix** 에만 적합하다.
창의·체감 작업에는 쓰지 않는다. `[auto]` 백로그는 본질적으로 얇으며(현재 ~2-3개), 흔한 종료 사유는
`DONE`(소진)이 아니라 **`MAX_NO_PROGRESS`** 다 — 정상이다. 효율을 내려면 **실행 전에 `[auto]` 항목을
한 묶음 seeding** 하라(회귀 테스트 백필, codemod, lint/type 부채 정리, stale-header 정리 등).
...
무인 루프. 회차마다 `claude -p "$(cat PROMPT.md)" --permission-mode acceptEdits --settings bin/overnight/overnight-settings.json --output-format json` 실행.
...
2. 성공이 아닐 때만 limit 텍스트(`usage/session limit`, `rate limit`, `overloaded`, `hit your …` 등) 검사 → `limit`.
...
경계가 겹치지 않게 분리 — LOOP의 각 단계를 담당(스킬은 `.claude/` 하위라 git 미추적·머신 로컬):
| 스킬 | 단계 | 책임 |
| --- | --- | --- |
...
`git push`·`curl`/`wget`·`rm -rf`·`sudo`·파괴/온라인 make(`infra-*`/`db-*`/`smoke`/`test-db`/`test-e2e*`/
`narrative-smoke`(비-fallback)/`visual-worker*`/`dev-*`/`streamlit`/`api`)·Web*·MCP(github/playwright)**.
...
GATE_CMD="make smoke-local" bin/overnight/run.sh &   # 런타임-flow 야간
...
- **dirty tree 오발**: 시작 시 워킹 트리가 dirty면 1회차가 잔여물 복구로 빠진다(또는 red면 STOP).
  첫 실행 전에 트리를 비워라(현재 미커밋 오프닝 배치 등).
- **브랜치 드리프트**: 커밋은 체크아웃된 브랜치에 쌓인다. `/overnight-report`가 브랜치를 명시한다. 전용 브랜치 권장.
- **스킬 git 미추적**: `.claude/`가 gitignore라 4종 스킬은 머신 로컬. 새 머신에선 스킬을 다시 둬야 한다.

## 6. 이 repo 적용 범위
적용 이력·실측 효과는 `docs/PROGRESS_LOG.md`에 회차 커밋과 함께 기록된다(이 doc은 설계 설명만 유지).
`[auto]` 후보의 정직한 triage는 `docs/NEXT_PLAN.md`의 자동화 태그를 본다.
