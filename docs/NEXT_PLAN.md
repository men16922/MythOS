...
Authority QA: `docs/test/neo_seoul_live_qa.md`. **Latest deploy = `mythos-api-00037-cj2` (2026-07-10)** — full stack + Se-rin fix trio #4/#5/#6 + ping/legend (`d175522`) + tae_o/han portrait-flash (`6e5f13f`) + **choice-gated stat voices + persona cast (`51b2264`) + companion appearance guardrail (`9bfce0d`) + Se-rin variant-opening combat-spawn root fix (`31bdceb`)**. Deploy hygiene: use `make deploy` (pins .env project; ambient gcloud config once drifted → stray service in claude-study-501117, deleted). Completed 07-04/05 gameplay/companion/cloud-runtime/model-routing/Kiro work → `docs/COMPLETED_SUMMARY.md` M55-M56.
