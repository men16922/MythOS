...
- **Image continuity**: lever #1 SHIPPED (`9e6e5b3` style preamble + `appearance` ×7) · lever #3 SHIPPED (`b0d7567` prod model → `gemini-3.1-flash-image` via `generate_content` + curated-portrait reference; deploy MUST set `IMAGEN_MODEL=gemini-3.1-flash-image`, Gemini quota is regional not per-model so likely no increase needed — observe 429s). `[ ]` `[auto:codex]` **lever #2** = curated key-beat anchor coverage — put AUTHORED art on recurring beats (route-node `anchor`/`image_sequence` refs) so those turns skip generation and always show the same fixed art; criterion `make validate-content` clean + referenced assets exist. **Art + wiring both codex** (오너 지시 "큐레이션 키아트=codex" 2026-07-06 — 기존 큐레이트 스타일 매칭 위해).
