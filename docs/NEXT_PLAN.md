...
- `[ ]` `[auto]` **equip-lang API regression test**: add a unittest asserting the `/api/v1/loops/{id}/equip` endpoint returns a localized snapshot when the request carries `lang="en"` (EN strings, not KO) and stays KO by default — the 2026-07-04 equip KO-leak fix (`EquipRequest.lang`) has no API-level regression lock yet. Done = new test in `tests/` + `make check` green.
