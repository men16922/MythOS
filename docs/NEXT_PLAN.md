...
- `[/]` `[auto:claude]` frontend god-component decomposition (App.tsx·CombatCinema): extract custom hooks/modules **one slice per iteration**, behavior-preserving. Done = `make check` green + post-commit AGY live-QA not FAIL/NEEDS (auto-screened, §3.4.1). _Progress: slices 1–13 + 15–17 done; **slice 18 A `useSaveLoad` DONE LOCALLY 2026-07-26** (App.tsx 979→956; isolated restore→resume smoke PASS; `make check` 1166; commit/post-commit AGY pending). **slice 14 (`useViewModels`) HUMAN-REVERTED (`4d88b80`) — DO NOT re-attempt verbatim.** Remaining survey choices (`docs/plans/2026-07-25-app-decomposition-slice18-candidates.md`): B `useEpiphanyBanner` next · C `useTabs` width-risk · D `useItemNotice` RESERVED for held-out bank. Owner picks B or closes the App.tsx half._
