...
- `[/]` **DS2 UNBLOCKED 2026-07-08** (owner sign-off, review packet `docs/plans/2026-07-08-ds2-sample-migration.md`: G1 size remap ✓ · G3 bake glow ✓ · G2 accept 6→8px ✓). **DS2-api + DS2-a + DS2-b DONE** (`make check` 953, emulator computed-style parity vs `.panel`): api = size scale remapped so default size-2 = 16px + glow baked into `.surface-surface` (+ DS2-b: Surface forwards `...rest`/`id`); a = `StatusPanel`; b = aside cluster (OperationMap main+fallback, Save/RunHistory). Remaining:
  - `[ ]` `[auto:claude]` **DS2-c … DS2-n** migrate the remaining ~25 `.panel` sites, **one cluster per slice** (combat · intro/onboarding · modals · dashboards; the 2 `<details className="panel">` chips + bespoke non-`.panel` classes last — need a Surface `as` prop or stay `<details>`), behavior-preserving. Done = `make check` green + post-commit AGY live-QA not FAIL (objective refactor, auto-screened).
