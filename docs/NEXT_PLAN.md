...
- `[/]` **DS2 UNBLOCKED 2026-07-08** (owner sign-off, review packet `docs/plans/2026-07-08-ds2-sample-migration.md`: G1 size remap ✓ · G3 bake glow ✓ · G2 accept 6→8px ✓). **DS2-api + DS2-a + DS2-b + DS2-c DONE** (`make check` 953, emulator computed-style parity vs `.panel`): api = size scale remapped so default size-2 = 16px + glow baked + Surface forwards `...rest`/`id`; a = `StatusPanel`; b = aside cluster (OperationMap ×2, Save/RunHistory); c = StoryPanel cluster (board/roster/scene-image/narrative-script). Remaining:
  - `[ ]` `[auto:claude]` **DS2-d … DS2-n** migrate the remaining ~21 `.panel` sites, **one cluster per slice** (intro/onboarding · modals · dashboards; the 2 `<details className="panel">` chips + bespoke non-`.panel` classes last — need a Surface `as` prop or stay `<details>`), behavior-preserving. Done = `make check` green + post-commit AGY live-QA not FAIL (objective refactor, auto-screened).
