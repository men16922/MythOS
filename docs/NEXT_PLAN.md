...
- `[x]` **Companion growth (3 channels)**: DONE 2026-07-04 — bond tiers (affection→HP/stats), achievement `allies[].upgrades` (all 6 neo-seoul allies), party-targeted boons ×3 (`companion_growth.py`, 10 tests). Remaining: `[ ]` `[manual]` UI surfacing (bond tier/upgrade badges on character sheet + codex) + balance feel.
