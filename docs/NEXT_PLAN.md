...
- `[blocked]` `[auto:claude]` npc_agenda 주체 무결성: 모든 `npc_agendas` 키가 `characters[].name`에 실재. 완료 기준: `test_content_integrity.py`에 테스트 추가, green 또는 Blocker(현재 `최적화 명단 대상자` 미스매치 예상 → Blocker면 사람 triage). **Blocker 2026-06-15(결정론적 설계 모호 → 사람 triage)**: strict invariant가 정당한 설계 이유로 RED — neo-seoul `최적화 명단 대상자`=의도된 추상 주체 + glass-library `characters[]` 공백(어젠다 3종 전부 미스). green화는 콘텐츠편집(금지) 또는 allowlist 고안(무인 검증불가 설계판단) 둘 다 mandate 위반. 재정의안: "캐릭터-참조 키 정확일치 + 선언된 추상 주체 allowlist 허용". 상세 PROGRESS_LOG 2026-06-15.
