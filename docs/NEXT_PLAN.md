...
- `[/]` P0 — live LLM 장기 세션 QA: fallback 12선택 + 실제 gemma4 14턴(in-process 드라이버, 2026-06-08) 통과.
  기술 파이프라인 양호(파싱 예외 0·선택지 상존·전투 후 재개·멈춤 없음). 남은 것: **F1 반복**(위치 불변 시
  도입부 감각 묘사 재사용 — 프롬프트 강화 후보), **F2 전투 빈도**(9장면 4전투·시작부 연속 — 빈도/연속 튜닝
  검토). 주관 항목은 사람 플레이(`docs/neo_seoul_live_qa.md`).
