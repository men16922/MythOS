...
- `[/]` **Owner rulings on the banked arm** — `[x]` the **22+25 two-build split does not disqualify it** (owner, 2026-08-13): registered in `SPLIT.json` `promotion` with `admitted_at`, hash-verified, bank now 3 samples (DECISIONS 2026-08-13). `[x]` **rubric scored** (`outputs/evals/20260814-005948/`): new sample 3/5, both 07-28 samples 2/5. `[ ]` `[manual]` the subjective **ending/overall feel verdict** is still open — and is now the *only* usable basis, because the score cannot carry the decision (below).
- `[ ]` `[manual]` **The promotion rubric cannot currently decide a promotion — owner call on what to do about it.** Re-scoring two hash-frozen, byte-identical samples returned 2/5 where they returned 3/5 in July: ~1 point of judge variance on unchanged input, the same size as the effect being measured. Options, none started: score each sample N times and take a median (N× judge cost), pin the judge model/version in `narrative_judge.py` (currently bare `claude -p`, so the engine drifts under it), or drop the numeric gate and keep the rubric as issue-finding only. The `issues` lists were substantive and are worth keeping either way. **Provenance is already recorded** (`provenance.json` + report header: judge argv, judge version, rubric SHA-256), so the *next* comparison can at least attribute drift — the July/August pair cannot, retroactively.
- `[x]` `[auto]` **Per-turn token usage logged** — closed 2026-08-14: no provider recorded token counts, which is why the promotion run had to use a documented ~$1.0/loop instead of a measurement. Gemini `generate`/`stream` now record usage into the `loop_id`-keyed turn log via `mythos_narrative/usage.py`; stream chunks are cumulative (counted once) and a retried turn sums both billed calls. `JsonFormatter`'s allowlist was dropping the fields and is fixed. `make check` **1285**. Residual `[ ]` `[auto]`: Ollama reports `prompt_eval_count`/`eval_count` and still records nothing (local dev, free — low value); no USD price table is applied anywhere.
...
- `[x]` `[auto]` **Clue counter + combat scene location** — closed: `_clues_collected` dropped the store's `limit` (default 8) and ignored shard kind, pinning the CLUE MATRIX gauge at 8/16 and desyncing the displayed Insight from the ending resolver's score; and combat scenes served the raw encounter id as `location`. The archived `clues_collected: []` was correct all along. `make check` 1218 (archive `progress-2026-08.md`, 2026-08-08, `b3143d8`).
- `[x]` **Residual objective/axis display gaps — CLOSED 2026-08-09.** Every reported symptom was a deterministic defect, not authoring. `[x]` **Portrait attribution was a defect**: the possessive guard rejected `X's voice` (the ordinary EN attribution), so IX matched nobody and the line fell through to Han; fixed and locked in both languages, owner 07-11 regressions re-verified dead. `[x]` **the reported symptom itself**: with two characters named, scenario array order decided the speaker; the speaker is now the name carrying a speech cue, and both portrait surfaces share one judgment. `[x]` **also found**: `"…," X says.` was not recognised as dialogue at all (33% of quoted spans in the arm). 16/16 probe (PROGRESS_LOG 2026-08-09). `[x]` **`CURRENT OBJECTIVE` missing — reproduced and fixed 2026-08-09**: the earlier ruling-out measured the server, where `chapter_goal` does cover all five phases; the defect was in the client. Only the mobile layout resolved the line as `objective || chapter_goal` — desktop rendered act goal and current objective as independent rows, so an omitted `objective` (optional in the scene schema, no default anywhere) dropped the CURRENT row while the strip still looked populated. Both layouts now share one resolution; locked by `tests/test_objective_strip_fallback.py`, `make check` 1264 (PROGRESS_LOG 2026-08-09). `[x]` **also a defect, not authoring**: both symptoms were one bug — `_choice_axis` matched a Korean-only vocabulary, so on an EN loop the axis collapsed onto an `intent` fallback that meant "Help people" (96/98 labels), and 43/45 multi-choice scenes shared one axis. Fixed with EN vocabulary, word-bounded keywords, and no chip when the label says nothing; 43/45 → 14/45 (PROGRESS_LOG 2026-08-09). `[x]` **chip coverage decided and shipped 2026-08-09** — owner adopted the recommended three families (traversal/sabotage/fight), `spoof → data` held. Chipless **34% → 10%** over 292 banked EN labels at exactly 1 reclassification; a per-term audit dropped `brace`, `run` and `draw your weapon` before implementation because each read a real label wrongly. Contrast improves on two arms and worsens by one scene on the third — the cost of coverage, recorded rather than smoothed. Evidence `docs/plans/2026-08-09-value-axis-vocabulary-coverage.md`, `make check` 1268. `[x]` `[auto]` follow-up **closed**: `civilians`/`survivor`/`survivors` added (chipless 30 → 28, 4 labels change, no new mislabel). The general "add every inflection" pass was measured and **rejected** — of the nine inflected forms that occur in the sample, seven change nothing and two (`decrypted`, `signals`) would misclassify real labels; `crowd` stays out and is now re-pinned by a test. `make check` 1270.
- `[x]` **Flee odds are invisible** — closed: `_flee_preview` mirrors the resolver (0/45 mismatches across agility 0-8 × adjacent 0-4) and the button shows the %. Rendered on the non-fallback simulator: `85%` → `75%` as a drone closes. `make check` 1222 (archive `progress-2026-08.md`, 2026-08-08, `4ca1d80`). `[ ]` `[manual]` residual: whether the chip is legible mid-fight at 390px is a feel verdict.
- `[x]` **Author low-risk encounters** — closed: tier 1 went from one encounter to four (`ration_line_watch`, `stray_incinerator`, `refuge_perimeter_probe`), so the never-wins path serves 20/20 ambient beats across four fights instead of 1 fight + 19 skips. The balance simulator rejected the first draft at 0.93–0.98 solo; retuned compositions sit at 0.58–0.77 (`patrol_ambush` = 0.58). `make check` 1225 (archive `progress-2026-08.md`, 2026-08-08). `[ ]` `[manual]` residual: whether the new fights read as distinct in play is a feel verdict.
- `[x]` **Korean-only matching logic on an EN-default product** — a defect *class*, found after the value-axis bug turned out to be an instance of it. **All six instances closed 2026-08-09**; the shared rule now lives in `mythos_core/text_match.py`, and a follow-up sweep of `src/mythos_ui/` found **no live instance** in the frontend (`termGloss.ts` already correct; `combatCinemaSkills.ts` 39/39 on the real catalog, latent trap documented in place). Each was reproduced before being fixed:
  - `[x]` **combat trigger** — closed: EN narration matched none of the four Korean phrases, so `combat_imminent` was raised with no fight behind it. EN phrases added, kept specific enough that "You avoid the ambush" still does not trigger; 7/7 both languages (PROGRESS_LOG 2026-08-09).
  - `[x]` **opening cinematic gate** — closed: fired on 4/9 EN opening turns (hung on the literal `C-17`), now 8/9 bilingual; a second unbounded matcher in the same function (legacy `character_map` fallback) was found by the regression test and fixed too (PROGRESS_LOG 2026-08-09).
  - `[x]` **`visual_service` unbounded character keyword** — closed: *change*/*handle*/*channel* all bound Han's reference image. Fixed via a shared `mythos_core/text_match.py` (`keyword_hits`/`mentions`/`name_mentions`) that the four duplicated copies now route through; the first attempt regressed Korean (`한이` stopped matching), so the repo's existing particle-bounding rule was promoted instead of dropping one-syllable names. 9/9 on real scenario data, 13 new tests (PROGRESS_LOG 2026-08-09). `[ ]` `[manual]` residual: the visible effect is on generated art, so it is unverified in a browser.
  - `[x]` **`_sfx_to_prose`** — closed: English cues returned hard-coded Korean sentences on the main JSON path; both languages authored, replacement follows the line's language (PROGRESS_LOG 2026-08-09).
  - `[x]` **unbracketed SFX pattern ate the sentence** — closed: `[A-Z0-9 _-]+` under IGNORECASE ran to the sentence end and deleted the trailing clause, in English only (Hangul had been acting as the boundary). Label is now uppercase-only; 5/5 keep their clause (PROGRESS_LOG 2026-08-09).
...
- `[x]` **`_commit_scene` pure reducer** — closed 2026-09-05 (`fc70a2f`: `_advance_narrative_state` → `_NarrativeAdvance`, locked by a raise-on-write store test). Was: — extract the ~280-line loop-state reduction (materialize → encounter map → route advance → beat/twist) into `_advance_narrative_state(...)` with no store access; `_commit_scene` becomes filter → engine → reduce → transaction → snapshot → combat handoff. *Done when the reducer has a unit test without the store fake and `make check` is green.*
- `[x]` **Snapshot builders** — closed 2026-09-05 (`_snapshot_from_loop`; `_commit_combat_turn`'s literal kept: it serves `result.*` from the turn, not a re-planned state). — 7 near-identical read-only `RuntimeSnapshot` constructors (`inscribe_echo`, `choose_boon`, `exchange_material`, `_redirect_to_active_combat`, `resume`, `load_save_slot`, `_snapshot_from_loop`) → one `_snapshot_from_loop(player, loop, options, *, scene=None)`; `_commit_combat_turn`'s inline combat literal → `_combat_snapshot`. *Done when existing resume/load tests pass unchanged.*
- `[x]` **Fetch-once progress facts** — closed 2026-09-05 (`_progress_facts`). — `_clues_collected` + `_epiphanies_unlocked` re-read shards/events/progression per snapshot (3× shards per `choose`; `archive()` fetches shards three times). *Done when one `_progress_facts()` accepts pre-loaded lists and the store fake counts ≤1 shard read per transition.*
- `[x]` **Intent telegraph vs `_enemy_turn`** — closed 2026-09-05 (stun/hack → `idle`, freeze → no move, taunt pool). — `update_enemy_intents` ignores freeze/stun/hacked/taunt/boss-skill turns, so the preview shows a CC'd enemy attacking. Share one `_plan_step` and emit `idle` for CC'd units. *Done when a stunned enemy's intent is `idle` in a test.*
- `[x]` **WS pins a pooled DB connection per open socket** — closed 2026-09-05 (`store.close()` after each `_run_stream`). (`Depends(get_service)` per socket; pool `max_size=10`): `store.close()` after each `_run_stream`. *Done when a test shows the connection returned between messages.*
...
- `[x]` **`requested_next_phase: "archive"` bypasses the soft-defeat guard** — closed 2026-09-05. in `mythos_loop/engine.py`; route it through `_archive_requested`. *Done when a soft-defeat loop ignores a model-requested archive in a test.*
- `[x]` **Refusal cannot clear a pre-seeded `met_se_rin`** — closed 2026-09-05 (`remove_flags` in the delta). (flag merge is union-only; loop 1 pre-seeds it). Decide: `remove_flags` in `_merge_state`, or stop pre-seeding. *Done when "Refuse her hand" on loop 1 yields `refused_se_rin` without `met_se_rin`.*
- `[ ]` `[auto:claude]` **Route replay re-scores passed anchors with later flags** (`route_runtime.py:177-206`) while node rewards were applied for the original perspective. Honour stored `active_perspectives`. *Done when the existing ending-tally test plus a flag-added-later case pass.*
- `[/]` **Frontend `NarrationReveal` + `combatView.ts`** — `NarrationReveal` closed 2026-09-05 (`ac8dbd3`, external store + `useSyncExternalStore`, verified in the local SPA); `[ ]` `[auto:claude]` `combatView.ts` remains (the three faction palettes differ on purpose? — check before unifying). — the typewriter tick re-renders the whole tree (a leaf owning `displayedNarration`), and `factionColor`/sprite fallback/HP ratio/`isAlive` are duplicated 3-15× across six surfaces. *Done when `make frontend-build` + Playwright E2E pass.*
- `[x]` **`combatCanvas` per-frame layout** — closed 2026-09-05 (+ latest-frame redraw on sprite load); desktop browser check done 2026-09-05 (simulator: sprites/cover/hazards/telegraph/skill VFX render, no console errors); `[ ]` `[manual]` residual: phone check. — `getComputedStyle` + 4× `matchMedia` + unconditional `canvas.width/height` reassign per draw; cache per animate/pointer session.
- `[/]` **Lesser substring matches** — `serializers` done 2026-09-05; `ending_resolver`/`audio_service` deliberately left (scoring semantics / no boss ids) — `[manual]` owner call if wanted. — `ending_resolver.py:111` (`metric in flag`: `lost_humanity` scores Humanity), `audio_service.py:63` (`"boss" in encounter_id`), `serializers._calculate_zone_risk`; route through `mentions()`. *Done when each has a negative-case test.*
