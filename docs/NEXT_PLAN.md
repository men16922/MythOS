...
- `[/]` **Korean-only matching logic on an EN-default product** — a defect *class*, found after the value-axis bug turned out to be an instance of it. `[x]` Se-rin accept/refuse in `mythos_loop/engine.py` (gameplay: EN players could not refuse; PROGRESS_LOG 2026-08-09). Open instances, all measured-by-reading and needing a reproduction before fixing:
  ...
  - `[ ]` `[auto]` **`visual_service.py:748` opening cinematic gate** — three of four alternatives are Korean; the EN path depends on the model writing the literal `C-17`, so the hand-authored opening reference images are intermittently skipped and opening image identity drifts. Completion criterion: the gate fires on EN opening narration that names Se-rin or a drone, `make check` green.
  ...
  - `[ ]` `[auto]` **`parser.py:234` `_sfx_to_prose`** — inverse polarity: the matcher is English but every replacement is a hard-coded Korean sentence, so an EN scene with an `[SFX: GLITCH]` marker gets Korean spliced into English prose. On the main JSON path. Completion criterion: EN loops get EN replacement prose, `make check` green.
