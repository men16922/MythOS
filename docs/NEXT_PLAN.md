...
- `[/]` **Implicit context caching v2 (diagnosis corrected — NOT blocked)**: implicit caching WORKS with real turn spacing (20s; the earlier 0 was cache-write propagation, not absence). Hit condition = shared prefix ≥ **2,048 tok (2.5)** / **4,096 tok (3.5)**; systemInstruction counts. Current stable prefix ~1.5–1.8k → 0 hits on both (measured, real path). v2 plan: stable-first note assembly (authored rules→bible→grant/persona→dynamic anti-repeat→route steering) + sliding windows (memories/shards) after `loop` in the payload + fix the `MAX_PROMPT_NOTES=8` window so rule notes actually render → est. ~2.4–2.5k stable = 2.5-flash passes (input −25–30%). 3.5's 4096 still needs the KO directive-pack-in-system redesign (~1k tok short; quality QA). Watch: gemini-3 9k–17k prompt cache dead zone (python-genai#2064). Detail `docs/plans/2026-07-04-gemini-2.5-vs-3.5-eval.md`.
- `[ ]` **`MAX_PROMPT_NOTES=8` truncation review (found during caching work)**: 21+ notes are built per turn but only the last 8 render — the language/cinematic/naming/causality rules are silently dropped on normal turns. Needs a deliberate window/priority policy (quality-sensitive; raising it also raises input tokens).
