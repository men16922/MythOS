...
- `[/]` **Residual objective/axis display gaps** — re-examined 2026-08-09, splitting into three outcomes. `[x]` **Portrait attribution was a defect**: the possessive guard rejected `X's voice` (the ordinary EN attribution), so IX matched nobody and the line fell through to Han; fixed and locked in both languages, owner 07-11 regressions re-verified dead. `[ ]` `[manual]` **residual on the same symptom**: with two characters named, scenario array order still decides the speaker — fixing it needs a speech-cue rule and no simple ordering works in both directions, so it is an owner call (evidence in PROGRESS_LOG 2026-08-09). `[~]` **`CURRENT OBJECTIVE` missing — not reproducible**: `chapter_goal` covers all five playable phases, so the strip cannot go empty on a well-formed loop; needs the specific scene ids to go further. `[ ]` `[manual]` **still open as authoring**: both choices on one axis (2 turns), an evasion action labelled `Help people`.
