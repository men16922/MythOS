...
- **Track M — mobile foundation (P0, prerequisite; do first)**:
  ...
  - `[x]` `[auto:claude]` **M3 DONE 2026-07-08** hover-only `title=` (~20 sites) → tap-openable tooltip. **T4a axis chip DONE** (`AxisChip` popover in `ChoicePanel.tsx`; unit test `test_choice_axis_chip_tooltip_is_tap_openable`; `make check` 917 green). **Combat-skill tooltip DONE** (`SkillInfoTooltip` in `CombatControls.tsx`, the richest of its 3 title sites; unit test `test_combat_skill_tooltip_is_tap_openable`; `make check` 918 green). **`GameAside` no-click divs DONE** (`InfoPopover` shared component in `GameAside.tsx` converts the 4 sites with no click handler at all — route node, fog stub, minimap enemy cell, minimap tile cell — the highest touch-info-loss since they had zero tap affordance; unit test `test_game_aside_info_divs_are_tap_openable`; `make check` 919 green). Remaining sites judged out-of-scope 2026-07-08 (verified in source, not converted): `GameAside`'s 2 remaining sites (route-map expand button, status-hints toggle button), `CombatControls`×2 (attack/item buttons), `StoryPanel`×3 (legend/zoom buttons), `HeaderBar`×2 (lang/concise/bgm toggles) are all already-tappable buttons with visible text/icon label + working `onClick`, where `title=` is a purely supplementary hover hint (e.g. disabled-reason, item effect) — same reasoning across all sites, no hover-only info loss on touch. AGY @390px tap check still pending post-commit auto-screen for the T4a/combat-skill/GameAside conversions (M3 itself is code-complete; the screen is a verification formality, not remaining work).
