...
- `[/]` **2-티어 전투 컨트롤** — 설계 `docs/plans/2026-07-12-two-tier-combat-control.md`. **핵심 3슬라이스 DONE**: slice 1 EMP 셀 타겟팅 · slice 2 **명중%·피해 예보 칩** (`a164f81` — `_attack_preview`가 실제 공격 수식 미러, 🎯65% ⚔6-16 🛡 형식) · slice 3 **적 인텐트 호버 렌즈** (`de00355` — 적 탭/호버 시 그 적의 텔레그래프만 강조+타 인텐트 딤, 인스펙터 "⚔2d6 → 세린"). `[x]` **slice 1 확장 DONE (`e67dafa`)**: 위치 의존 스킬(밀기/당기기/광역/스턴)에 🎯 조준 토글 — 보드에서 적을 짚으면 변위 도착지 화살표/폭발 링/💫 프리뷰 후 시전, 기본 버튼은 자동 타겟 유지(캐주얼 불변). 잔여: slice 4(턴 순서 스트립)는 `[manual]` 오너 체감 판정 후.
- `[/]` **상태이상 시스템** — 설계 `docs/plans/2026-07-12-status-effects-design.md`. **slice 1 DONE (`b55b37b`)**: `status_effects` 채널 + 연소(턴 시작 1d4 DoT, 행동 전 사망 가능)/부식(장갑 -2) + `applies` 스킬 라이더 + 보드 배지 pill 행(💫/🔥/🧪) + KO/EN 로그 + 직렬화 왕복 tests. `[x]` **slice 2 DONE (`9ff6419`)** + `[x]` **slice 3 배선 DONE (`1937a84`, 오너 "같이 진행해")**: 적 무기 라이더(소각기=🔥토치·신호추적기=💧🧪산성분사·집행유닛 충격봉=⚡) + 소이/냉각 수류탄(XCOM 셀 투척, status_grenade 범용 브랜치, 테스트 킷 포함) + **보드 임팩트 오버홀**(낚아채기 2중 쇼크웨이브+플래시+"밀려남!" 플로트+셰이크 2배 · 상태 부여 순간 색상 링 폭발+배지 상승 · AoE 셰이크 1.7배 · 🕹 배신 타격 풀스크린 컷인 승격 · 배지=아이콘 이미지 규약 `status/<id>.png`+필 폴백). 잔여: `[ ]` `[auto:agy]` **상태 아이콘 7종 + 수류탄 아이콘 2종** (아래) · `[ ]` `[manual]` 임팩트 체감 재판정 + 수치 밸런스(부여 턴수/빈도) 튜닝.
- `[x]` `[auto:agy]` **상태이상 아이콘 7종 DONE (`8e22944`)** — `resources/neo-seoul/status/*.png` 네온 아이콘 생성(수류탄 아이콘 2종은 `items/`에 emp 복사 플레이스홀더 잔존). `[ ]` `[auto:agy]` **소이/냉각 수류탄 전용 아이콘** (현재 emp_grenade 복사).
- `[x]` **전투 완성도 배치 DONE (`abe2d6a`/`84210e3`, make check 1041)** — 직접 로컬 시뮬(chrome-devtools) 테스트로 발굴: 전 스킬 시네마 카드(레지스트리 5/~20→전체) · EMP 계열 1d4 `shock_damage` 라이더(순수 제어라 피해 0이던 것) · 과부하 스플래시 절반+공격자당 컷인 dedup(적2 원샷/2회 발동 해결) · EMP/냉각 수류탄 실폭발 VFX(피해 0 투척물이 instant 경로로 빠지던 것 포함) · 조준 사거리 틴트 · 신규 수류탄/무기 EN 용어집. **fallback→정적 전투 분리 시도는 REVERT(설계 의도).**
- `[x]` **전투 비주얼 오버홀 V1-V6 DONE (2026-07-12 session #14, `8b86423`/`075da28`/`6eaa975`, make check 1041)** — 오너 확정: "시각적으로 별로임" = 4개 영역 전부 + 카메라 팬/노드 배경 요청 2건. 카메라 드래그 팬+센터링 버그(우측 스프라이트 잘림) · 노드 색조 배경(biome=encounter id, 아트 훅 `combat/backdrops/<biome>.png`) · 셀 단위 AoE/다이아 링/모서리 셰브런 사거리 · 배지 칩(이름 위 원형 칩) · 컷인 임팩트 슬래시+스피드라인+수류탄 아트 카드 · 스킬 글리프/아이템 썸네일 전체 커버. 설계 `docs/plans/2026-07-12-combat-visual-overhaul.md`, 증거 `outputs/vis-diag/`.
- `[ ]` `[manual]` **오너 체감 재판정** — 배포 후 `docs/test/neo_seoul_live_qa.md` A-1(조준/⚡감전/🕹조종)·A-3(임팩트 오버홀+적 상태이상+수류탄) + **비주얼 오버홀 체감**(팬/배경 톤/셀 링/배지 칩/컷인 슬래시) + **session #15 신규**(충돌 슬램 💥 · 냉각 폭발+피해 · 상태 중첩 체감 · 로컬 BGM 무음). 판정이 아래 agy 아트 시드 착수 게이트. (오너가 로컬에서 낚아채기/상태부여/배신컷인 [x] 선기록 — QA doc 미커밋 오너 사본.)
...
- `[x]` **상태이상 중첩 + 다중 동시 적용 DONE (`2d94ad6`, session #15)** — 누적(cap 6 튜너블, 스턴 포함) + 다중 동시 정합 + upkeep DoT 사망 턴 정합; unittest + make check 1058 + 시뮬 non-fallback 확인. **동일 커밋에 오너 추가 지시 3건**: 충돌 슬램(변위가 벽/full-cover 구조물/유닛에 막히면 1d4 추가 피해 + 보드 버스트; full cover는 강제이동만 차단) · 냉각 수류탄 1d4 · (반발 진단: 컷인 정상, 0칸 밀림이 실체 → 슬램으로 가시화).
- `[x]` **BGM 기본값 ENV화 DONE (`c300f1a`)** — `DEFAULT_BGM_ON` → open `/api/v1/client-config`; make api/api-cloud = false; 오프닝 강제 재생 제거.
- `[x]` `[auto:codex]` **트리아지 2건 DONE (codex 병렬 worktree, 검수 후 cherry-pick)**: 린위에 승리 라인업(`.slice(0,3)` 절단, `0066319`) · 전리품 필 raw id(KO/EN 표시명 11종+폴백, `89d05d8`).
