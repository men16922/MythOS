...
- `[/]` **Frontend `NarrationReveal` + `combatView.ts`** — `NarrationReveal` closed 2026-09-05 (`ac8dbd3`, external store + `useSyncExternalStore`, verified in the local SPA); `[ ]` `[auto:claude]` `combatView.ts` remains — `factionColor`/sprite-fallback/HP-ratio/`isAlive` duplicated 3-15× across six surfaces (check whether the three faction palettes differ on purpose before unifying).
