...
- `[ ]` `[blocked]` `[auto:claude]` frontend god-component decomposition (App.tsx·CombatCinema): extract custom hooks/modules **one slice per iteration**, behavior-preserving. Done = `make check` green + post-commit AGY live-QA not FAIL/NEEDS (auto-screened, §3.4.1). Was `[manual]` (E2E-sensitive); now guarded by auto live-QA. _Progress (recurring): slice 1 = `hooks/useInGameEpiphany.ts` (2026-06-21 (l)); slice 2 = `hooks/useCombatBoard.ts` (combat board pointer/drag + tile inspector + zoom, 2026-06-21 (m)); slice 3 = `hooks/useTypewriter.ts` (narration typewriter state/refs/loop, 2026-06-21 (q)); slice 4 = `hooks/useGameSocket.ts` (WebSocket connect + auto-reconnect refs/openSocket/closeSocket, 2026-06-21 (r)); slice 5 = `hooks/useCombatCinemaQueue.ts` (cinema-queue state + prev/dispatched/pending refs + canvas-draw useEffect + onCinemaImpact/onCinemaFinish, App.tsx 1261→1102, 2026-06-21 (s)); slice 6 = `hooks/useSceneVisuals.ts` (sceneImageUrl/imagePlaceholderText state + visualTimeoutRef watchdog + clearVisualTimeout/onVisualStatus/resolveImage, App.tsx 1102→1069, 2026-06-21 (t)); slice 7 = `hooks/useSessionLifecycle.ts` (session lifecycle: handleStartGame/handleSimulateCombat/handleResumeGame + saveSessionMetadata, App.tsx 1069→927, 2026-06-28); slice 8 = `hooks/useDataLoaders.ts` (read-side loaders: loadSlotsAndRuns/loadCodex/loadSkillTree + handleLearnSkill, App.tsx 927→876, 2026-06-28); slice 9 = `hooks/useCombatRest.ts` (combat REST cluster: handleCombatAction/appendCombatLog/handleEquip/continueAfterCombat, App.tsx 876→820, 2026-06-28); slice 10 = `hooks/useSessionControls.ts` (handleScenarioChange/handleLeaveSession/handleSaveSlotSubmit + NEW shared `archetypes.ts`, App.tsx 820→797, 2026-06-28); slice 11 = `hooks/useSnapshotReceiver.ts` (handleReceivedSnapshot snapshot-apply + internal triggerCinematicEffects turn-0 motion, called from the WS type-switch, App.tsx 797→766, 2026-06-28); slice 12 = `hooks/useNarrativeStream.ts` (gameplay narrative stream: handleSocketMessage inbound type-switch + useGameSocket lifecycle now called inside the hook + beginStream/imageOpts/sendChoose outbound send; useDataLoaders+useSnapshotReceiver reordered above it for handleReceivedSnapshot scope, App.tsx 766→716, 2026-06-28); slice 13 = `hooks/useKeyboardChoice.ts` (number-key 1-9 choice-hotkey useEffect: picks active_scene.choices[n-1], gated by isChoiceDisabled, emits via sendChoose; isChoiceDisabled import moved into the hook, App.tsx 716→696, 2026-06-28). **slice 14 (codex/dev/tab view-model `useMemo` cluster → `useViewModels`) was committed green (`d7b3b35`) then HUMAN-REVERTED 8s later (`4d88b80`, 2026-06-28) — DO NOT re-attempt verbatim (would undo a deliberate revert; the extraction needed a 12-field props object = net-negative). Blocker #1 recorded (PROGRESS_LOG 2026-06-28). Track paused: a human must confirm whether App.tsx decomposition is done/paused, or name a different next slice with a clean few-prop boundary. **`[blocked]` 2026-06-28 (2nd encounter):** no human resolution since Blocker #1 + human moved focus to a new branch (`feat/en-ko-s0-language-plumbing`, EN/KO work) → formally blocked per the overnight twice-blocked rule; remove the tag after human review names a slice or closes the track._
