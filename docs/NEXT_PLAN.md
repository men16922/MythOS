...
- `[ ]` `[manual]` **Commit follow-up round + redeploy + live sign-off**: commit the uncommitted live-QA round (boss-victory ending fix, name guard, equip KO leak, compact card, IX meta_scaling, wet_dawn art), redeploy Cloud Run, then human fresh-loop play per the 🔴 checklist; human `git push origin main` (ahead 12).
