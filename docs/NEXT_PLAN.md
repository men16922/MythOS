...
- `[ ]` **전투 A/V 싱크** (3 code-verified vectors): (A) SFX cold-start(`new Audio` 매번, 프리로드 없음 → `setTimeout` 시각보다 지연) · (B) 투사체가 임팩트 큐보다 ~250ms 선착(800 vs 1050ms std) · (C) 연출 리플레이 중 로스터/인스펙터는 최종 턴-후 HP 표시. A+B cheap `[auto:claude]`(기준: impact 큐=비행 종료 ±50ms + `make check` green), C = 설계.
- `[ ]` **drone_scrap 무근거 지급**: 원인 = `prompts.py` JSON 계약 few-shot 예시가 `grant_items:["drone_scrap"]`(KO/EN) → 모델이 Success 턴마다 모방 지급(2회 목격). Fix 후보 `[auto:claude]`: 예시를 중립 placeholder로 + 허용 id는 지침만; 부수 = 직전결과가 raw id/한글명 혼용. 서사 체감 영향 → 오너 승인 후.
- `[ ]` **한 뜬금 전투 합류**: 원인 = meet-arc 노드("사이드: 러너의 지름길") **진입 시점** effect가 `met_han` 세팅(route_runtime 인과 순서) — GM 산문이 한을 등장시키기 전에 전투가 끼면 unlock∩flags로 캐스팅(+C3 합류 신호). Fix 후보: 합류 신호를 "산문에 등장한 적 없는 아군은 대기"로 강화 or meet-노드 directive가 등장을 강제. 설계 판단 → 오너.
