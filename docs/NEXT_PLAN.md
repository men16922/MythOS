...
- `[ ]` `[manual]` **Teaser #2 (version-up video)**: after sign-off — `git push` + version tag (video build = deployed rev), record on a clean tester key, upload with `docs/cbt/CBT_TEASER.md` metadata/shot list. If the video targets NEW-tester recruiting, build CBT onboarding P1 first so inflow lands on the improved first session.
