...
- `[x]` `[auto:claude]` **(N) 호감도 — `progression` 크로스-루프 이월**: meta progression에 `relationships` 적립/이월(insight 패턴). 완료 기준: 유닛테스트 + `make check` green. (L 선행, feature — 검수 필요) (2026-06-16 박제 — `MetaProgression.relationships` + `RunSummary.relationships`(archive서 `loop.state["relationships"]` 추출) + `evaluate_meta_progression` 누적(insight 동형, 0 prune) + migration 006 `relationships` JSONB + postgres `_PROGRESSION_DICT_COLS`. `RelationshipCarryOverTest` 8건, 414 green. feature — 아침 검수 필요)
- `[x]` `[auto:claude]` **(O) 호감도 — serializer 노출 계약 회귀 테스트**: 시드 L/M/N가 생성한 affection 데이터(라이브 `loop.state["relationships"]` + 크로스루프 `meta_progression["relationships"]`)를 API serializer가 surface하는지 박제(향후 state-field 필터가 조용히 드롭하는 dead-data 회귀 가드). 완료 기준: `test_api.py` 추가 + `make check` green. (2026-06-16 박제 — `ApiRelationshipSerializerTest` 3건: snapshot 라이브 노출·미설정 시 키 미생성·memory overview 크로스루프 노출. 417 green. "표시"의 auto-검증 가능 절반 — 프론트 게이지 UI는 `[manual]` 잔여.)
