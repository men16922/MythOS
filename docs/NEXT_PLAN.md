...
- `[/]` **Korean-only matching logic on an EN-default product** — a defect *class*, found after the value-axis bug turned out to be an instance of it. `[x]` Se-rin accept/refuse in `mythos_loop/engine.py` (gameplay: EN players could not refuse; PROGRESS_LOG 2026-08-09). Open instances, all measured-by-reading and needing a reproduction before fixing:
  - `[ ]` `[auto]` **`parser.py:506` combat trigger** — `start_combat` is detected from four Korean phrases plus `encounter_`; English narration ("Combat begins", "the drone opens fire") matches nothing, so the turn sets the bilingual `combat_imminent` flag and then no fight starts. Dual-model/plain-text path only (`ollama_model_story` set), so local-dev-conditional, not the cloud JSON path. Completion criterion: an EN narration fixture that currently yields `start_combat=None` yields the encounter id, `make check` green.
  ...
  - `[ ]` `[auto]` **unbracketed SFX pattern eats the sentence** — found while fixing the above: `sfx\s*:\s*[A-Z0-9 _-]+` under `IGNORECASE` matches lowercase too, so `"SFX: ALARM WAIL and the crowd scatters."` consumes the trailing clause. Completion criterion: the tail survives, `make check` green.
