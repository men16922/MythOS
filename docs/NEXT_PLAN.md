...
Authority QA: `docs/test/neo_seoul_live_qa.md`. **Latest deploy = `mythos-api-00038-v8d` (2026-07-10)** — full stack + Se-rin fix trio #4/#5/#6 + ping/legend + tae_o/han portrait-flash + choice-gated stat voices + persona cast + **companion-appearance thread closed** (guardrail `9bfce0d` · combat-spawn `31bdceb` · cutscene present-gate `10efaeb` · combat-callback `0f52e16`). Deploy hygiene: use `make deploy` (pins .env project; ambient gcloud config once drifted → stray service in claude-study-501117, deleted). Completed 07-04/05 gameplay/companion/cloud-runtime/model-routing/Kiro work → `docs/COMPLETED_SUMMARY.md` M55-M56.
