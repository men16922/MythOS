...
- `[ ]` `[auto:agy]` **상태이상 아이콘 7종 + 수류탄 아이콘 2종** — `resources/neo-seoul/status/{stunned,burn,corrode,acid,freeze,shock,hacked}.png` (32~64px 아이콘, 투명 배경, 기존 스킬 아이콘 스타일 일관; 배지로 유닛 머리 위에 뜸: 💫기절/🔥과열/🧪부식/💧용해/❄동결/⚡감전/🕹조종) + `resources/neo-seoul/items/{incendiary_grenade,cryo_grenade}.png` 전용 일러스트(현재 emp_grenade 복사 플레이스홀더). criterion: 파일 존재 + `make check` green (status 아이콘은 없으면 텍스트 필 폴백이라 부분 승격 가능).
