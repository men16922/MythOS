...
Authority QA: `docs/test/neo_seoul_live_qa.md`. **Latest deploy = `mythos-api-00034-8xj` (2026-07-10)** — full stack + mobile 대본 최우선 (#3) + #3b setup overflow + Se-rin fix trio #4/#5/#6 + ping/legend (`d175522`) + **tae_o/han portrait-flash fix** (`6e5f13f`). Deploy hygiene: use `make deploy` (pins .env project; ambient gcloud config once drifted → stray service in claude-study-501117, deleted). Completed 07-04/05 gameplay/companion/cloud-runtime/model-routing/Kiro work → `docs/COMPLETED_SUMMARY.md` M55-M56.

- `[/]` **Se-rin appearance — flag-carry RULED OUT, mid-loop reintroduction OPEN (2026-07-10).** Reproduced (`scratch/repro_serin.py`): a fresh loop-2+ variant carries **no met_/trusted_ flags + empty `_party`** even under a met+trusted meta → no flag-carry into new-loop initial state (owner-decided "stop carrying" fix had no target). One han loop confirmed clean. **But owner reports Se-rin still appearing mid-loop in a variant** (screenshot narration "…정세린이다") → **separate vector = model reintroduction during play** (story-bible/echo leak or a legit Se-rin node), not initial seeding. `[ ]` NEXT: inspect the affected loop state (turn her flag/presence appears; model `world_delta` vs route node) BEFORE any fix.
