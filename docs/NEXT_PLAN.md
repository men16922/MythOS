...
- `[x]` **Korean-only matching logic on an EN-default product** — a defect *class*, found after the value-axis bug turned out to be an instance of it. **All six instances closed 2026-08-09**; the shared rule now lives in `mythos_core/text_match.py`. Each was reproduced before being fixed:
