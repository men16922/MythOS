...
Authority QA: `docs/test/neo_seoul_live_qa.md`; scope: `docs/reports/2026-07-31-late-loop-repetition-scope.md`. **Latest deploy = `mythos-api-00086-4d5` (2026-09-05; the 39-commit fix bundle + the three review bundles `047fcf0`..`f1d22d6`; hold lifted, DECISIONS 2026-09-05).** Typed fallback evidence, combat pacing, structural novelty enforcement, and Gemini 3.1 image migration are live. The 2026-08-01 fresh-arm attempt (`loop_8b7a…60dc2`) was excluded at 13/14 (`parse_error`); it and the partial QA loop are evidence only.
...
- `[ ]` `[auto:claude]` **Snapshot builders** — 7 near-identical read-only `RuntimeSnapshot` constructors (`inscribe_echo`, `choose_boon`, `exchange_material`, `_redirect_to_active_combat`, `resume`, `load_save_slot`, `_snapshot_from_loop`) → one `_snapshot_from_loop(player, loop, options, *, scene=None)`; `_commit_combat_turn`'s inline combat literal → `_combat_snapshot`. *Done when existing resume/load tests pass unchanged.*
- `[ ]` `[auto:claude]` **Fetch-once progress facts** — `_clues_collected` + `_epiphanies_unlocked` re-read shards/events/progression per snapshot (3× shards per `choose`; `archive()` fetches shards three times). *Done when one `_progress_facts()` accepts pre-loaded lists and the store fake counts ≤1 shard read per transition.*
- `[ ]` `[auto:claude]` **Intent telegraph vs `_enemy_turn`** — `update_enemy_intents` ignores freeze/stun/hacked/taunt/boss-skill turns, so the preview shows a CC'd enemy attacking. Share one `_plan_step` and emit `idle` for CC'd units. *Done when a stunned enemy's intent is `idle` in a test.*
- `[ ]` `[auto:claude]` **WS pins a pooled DB connection per open socket** (`Depends(get_service)` per socket; pool `max_size=10`): `store.close()` after each `_run_stream`. *Done when a test shows the connection returned between messages.*
...
- `[ ]` `[auto:claude]` **`requested_next_phase: "archive"` bypasses the soft-defeat guard** in `mythos_loop/engine.py`; route it through `_archive_requested`. *Done when a soft-defeat loop ignores a model-requested archive in a test.*
- `[ ]` `[auto:claude]` **Refusal cannot clear a pre-seeded `met_se_rin`** (flag merge is union-only; loop 1 pre-seeds it). Decide: `remove_flags` in `_merge_state`, or stop pre-seeding. *Done when "Refuse her hand" on loop 1 yields `refused_se_rin` without `met_se_rin`.*
...
- `[ ]` `[auto:claude]` **`combatCanvas` per-frame layout** — `getComputedStyle` + 4× `matchMedia` + unconditional `canvas.width/height` reassign per draw; cache per animate/pointer session.
- `[ ]` `[auto:codex]` **Lesser substring matches** — `ending_resolver.py:111` (`metric in flag`: `lost_humanity` scores Humanity), `audio_service.py:63` (`"boss" in encounter_id`), `serializers._calculate_zone_risk`; route through `mentions()`. *Done when each has a negative-case test.*
