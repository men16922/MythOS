...
Authority QA: `docs/test/neo_seoul_live_qa.md`. **Latest deploy = `mythos-api-00068-76m` (2026-07-15; 100% traffic)** — on top of the portrait action dock and turn-order strip, it restores cloud images by pinning the directly verified `gemini-2.5-flash-image` after 3.1 returned 404. Next required action: owner REAL-DEVICE portrait combat pass — dock reachability/height(38dvh)/URL-bar 100dvh/notch + strip 가독성; observe the next real-loop image.
...
- **Image continuity**: lever #1 SHIPPED (`9e6e5b3` style preamble + `appearance` ×7) · cloud model is now `gemini-2.5-flash-image` via `generate_content` + curated-portrait reference (3.1 was unavailable and returned 404; deploy pins 2.5; watch app-path image success and 429s separately). `[x]` `[auto:codex]` **lever #2 DONE**: curated three-turn fixed-art sequences for night market, incinerator, Kai awakening, Spire gate, and IX; `make validate-content` clean + referenced assets exist. **Art + wiring both codex** (오너 지시 "큐레이션 키아트=codex" 2026-07-06 — 기존 큐레이트 스타일 매칭 위해).
...
- `[/]` `[manual]` **Key-beat hybrid enablement + A/B verdict — PREPARED 2026-07-14 (owner GO)**: agent-side work done — routing observability added (`model_override`/`key_beat` in the streaming-finished log + `JsonFormatter` whitelist; streaming-path routing source-locked, `test_keybeat_model.py` 18) + full protocol with owner commands in `docs/plans/2026-07-14-keybeat-hybrid-ab.md`. **LIVE + ROUTING VERIFIED 2026-07-14: `mythos-api-00066-blc`** — live play confirmed key-beat→3.5 (4/4 success) and normal→2.5 (1/1 success). En route found+fixed the 3.5 streaming whitespace-runaway silent fallback (`b55e933`: non-streaming retry + raw-evidence warning; probes `scratch/probe_keybeat_finish.py`). Remaining (owner): matched-loop A/B feel verdict (quality/repetition/errors/p50/p95/cost; watch `streamed payload unparseable` warning frequency) → documented keep/rollback in `DECISIONS.md`; rollback command in the plan.
