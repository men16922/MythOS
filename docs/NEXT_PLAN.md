...
- `[ ]` `[manual]` **오너 체감 재판정** — 배포 후 `docs/test/neo_seoul_live_qa.md` A-1(조준/⚡감전/🕹조종)·A-3(임팩트 오버홀+적 상태이상+수류탄). ⚠ **"시각적으로 별로임" 대상 확정 필요**(상태 배지 아이콘 / 조준·폭발 링 / 컷인 카드 / 보드 룩 중 무엇?) — 짚어주면 아트면 agy, 연출/색이면 코드로.
