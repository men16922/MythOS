...
- `[ ]` **Save-slot overwrite (user request 2026-07-05)**: SAVE UX에 기존 슬롯 덮어쓰기가 없어 저장이 슬롯 신규
  생성만 됨 — 슬롯 목록에서 "덮어쓰기" 선택(확인 다이얼로그) 지원. API/스토어에 슬롯 upsert 경로 확인 후 UI 연결.
  UI 와이어링은 design 스냅샷 후 `[auto]` 승격 후보 (criterion: `make check` green + AGY live-QA not FAIL/NEEDS).
