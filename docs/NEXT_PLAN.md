...
- `[x]` **Implicit context caching v2 — DONE (`1014e52`)**: stable-first note assembly + sliding windows after `loop` + `MAX_PROMPT_NOTES` 8→48 (rule notes were silently dropped on every normal turn — fixed). Measured: stable prefix 1.5k→2.55–2.85k tok (turns 2+, above 2.5-flash's 2048); first real-path cache hit recorded (`cached=3839`, 2.5, 20s spacing). 738 green. Remaining: `[ ]` `[manual]` live-play QA that restored rule notes (language/cinematic/naming/causality now reaching the model) read well — register/repetition feel may CHANGE vs before; `[ ]` 3.5's 4096 threshold needs the KO directive-pack-in-system redesign (~1.2k tok short; separate design, quality QA). Watch: gemini-3 9k–17k cache dead zone (python-genai#2064). Detail `docs/plans/2026-07-04-gemini-2.5-vs-3.5-eval.md`.
