...
### Combat A/V sync (owner feel report 2026-07-11 — DIAGNOSED, 3 code-verified vectors; fix pending owner scope pick)
- (A) SFX cold-start: every SFX = fresh `new Audio().play()`, no preload → audio lags the `setTimeout`-scheduled visuals (`useAudio.ts`). (B) projectile visually lands ~250ms before the impact cue/SFX/HP tick (t=800 vs 1050ms std, 400 vs 650 fast — `useCombatCinema.ts`). (C) replay-vs-truth: roster/TileInspector render final post-turn `finalizedSnapshot.combat` while the cinema still replays blow-by-blow; canvas shows a third interim HP (`onCinemaImpact`).
- `[ ]` `[manual]` owner picks scope: A+B cheap (`[auto:claude]`, criterion `make check` green + impact cue matches flight end ±50ms), C = replay-consistency design (interim board feeds roster/inspector until queue drains).
...
- `[x]` **Se-rin variant-loop appearance — ROOT FIXED 2026-07-10.** Chain: flag-carry into initial state RULED OUT by repro (`scratch/repro_serin.py`); loop-start companion guardrail added (`9bfce0d`); then the true root — the layer-0 opening anchor hardcodes `trusted_se_rin` and `effect` isn't variant-overridable, so han/kai loops inherited it (route_runtime applies mid-loop, bypassing the model-only validator clamp) → combat unlock `unlock_flags∩flags` spawned her. `_apply_anchor_variant` now strips se_rin contact flags from non-default variant anchors (`31bdceb`; default keeps them, her meet-arc side anchor untouched, static+dynamic covered). `[ ]` `[manual]` owner live retest on a fresh variant loop (no Se-rin in combat); in-flight/legacy loops = new loop or 1-time DB cleanup.
- `[x]` **tae_o/han portrait-flash — FIXED + DEPLOYED 2026-07-10 (`6e5f13f`)**: `detectSceneCharacter` fired on any keyword hit in scene text; tae_o `사냥개`(hound) / han `해커` matched ambient prose. Dropped both from detection `keywords` (aliases kept). Reviewed all 7 chars (no other offenders). Source-lock `CharacterDetectionKeywordIntegrityTest`.
