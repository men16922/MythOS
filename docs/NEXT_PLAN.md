...
- `[x]` `[auto:codex]` **엄폐 포즈 아트 정체성 재생성 DONE (7장 전부 — owner second-rejection feedback)** — all seven reviewed PASS sprites are promoted after per-character guard/idle reference checks; `outputs/cover-pose-regen/review.md` contains the five-point check table and `make check` is green (993 tests).
