...
- `[ ]` `[auto:claude]` **Frontend `NarrationReveal` + `combatView.ts`** — the typewriter tick re-renders the whole tree (a leaf owning `displayedNarration`), and `factionColor`/sprite fallback/HP ratio/`isAlive` are duplicated 3-15× across six surfaces. *Done when `make frontend-build` + Playwright E2E pass.*
- `[x]` **`combatCanvas` per-frame layout** — closed 2026-09-05 (+ latest-frame redraw on sprite load); `[ ]` `[manual]` residual: browser check on a phone. — `getComputedStyle` + 4× `matchMedia` + unconditional `canvas.width/height` reassign per draw; cache per animate/pointer session.
