...
- `[/]` **Korean-only matching logic on an EN-default product** — a defect *class*, found after the value-axis bug turned out to be an instance of it. `[x]` Se-rin accept/refuse in `mythos_loop/engine.py` (gameplay: EN players could not refuse; PROGRESS_LOG 2026-08-09). Open instances, all measured-by-reading and needing a reproduction before fixing:
  ...
  - `[ ]` `[auto]` **`visual_service.py:775` unbounded character keyword** — `any(kw in detect_text)` on ASCII keywords means *change*/*handle*/*channel* match Han's `한`/`han`, binding his reference image and appearance line into scenes he is not in. Same substring bug just fixed in `_axis_keyword_hit` and `_mentions`. Completion criterion: word-bounded ASCII matching, a probe showing *channel* no longer binds Han, `make check` green.
