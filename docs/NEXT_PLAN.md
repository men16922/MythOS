...
- `[ ]` `[manual]` **§1 Key-beat hybrid A/B feel verdict** — routing is LIVE + verified on `00066+` (key-beat→3.5 4/4, normal→2.5 1/1; whitespace-runaway retry fix live). Matched-loop comparison (quality/repetition/errors/p50/p95/cost; watch `streamed payload unparseable` frequency) → documented keep/rollback in `DECISIONS.md`. Protocol + rollback command: `docs/plans/2026-07-14-keybeat-hybrid-ab.md`.
