...
- `[ ]` `[manual]` **Clean cohort re-arm** — after the committed NumPy `<2.5` constraint and model-before-mypy preflight are green in a fresh disposable worktree, owner explicitly decides whether to spend on a new repair-0/subagents-0 cohort. Do not reuse the invalid run as arm A or enable repair first.
...
- `[/]` **WS5 harness operation**: plugin V2 cutover DONE 2026-07-19 (`COMPLETED_SUMMARY` M63). **1.3.0 locally released/pinned 2026-07-26** (durable graph/provenance/trajectory plus five-value verifier protocol; remote marketplace remains 1.2.0). `OVERNIGHT_REPAIR` stays **0**. Remaining:
  ...
  - `[/]` **held-out task bank — precondition for `OVERNIGHT_REPAIR=1`**: three docs/Python/UI tasks owner-ratified and frozen 2026-07-27 at `scripts/overnight/heldout-bank.md`. First cohort safely stopped 3/3 but was base-red and invalid for productivity; remaining: owner re-arms one clean repair-0 cohort, then a paired comparison before any default changes. Do not merge bank commits or tune against their content.
