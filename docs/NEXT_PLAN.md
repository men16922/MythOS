...
- `[ ]` **Vertex context caching for the narrative prompt (DECIDED 2026-07-04, next code track)**: cache the fixed prefix (system prompt + directives, ~5k tokens) via Vertex context caching so input cost drops before any `gemini-3.5-flash` rollout (turn input ~10k, 3.5 = $1.50/M in). Done = cache hit visible in `usage_metadata` (`cached_content_token_count`) + measured cost/turn cut + tests green. Then `[manual]` decide 2.5→3.5 (eval `docs/plans/2026-07-04-gemini-2.5-vs-3.5-eval.md`).
