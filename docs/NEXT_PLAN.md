...
- **Image continuity (lever #1 SHIPPED `9e6e5b3`: `visual_style` preamble + `appearance` ×7)**: `[ ]` `[auto:codex]` **lever #2** = curated key-beat anchor coverage (authored art + route-node `anchor`/`image_sequence` refs on recurring beats so more turns show authored art vs a fresh sample; criterion: `make validate-content` clean + referenced assets exist). `[ ]` `[manual]` **lever #3 (owner decision, URGENT)**: prod `imagen-3.0-generate-002` is DEPRECATED (shutdown ~2026-06-30, already passed) → migrate `VertexImageProvider.generate` to **`gemini-2.5-flash-image`**, which ALSO natively takes the curated portrait as a reference (fixes face drift + moves off the dead endpoint in one change; `google-genai 2.10` already supports it, ~cost parity). Research subagent 2026-07-11; key file `visual_service.py:263-291`.
