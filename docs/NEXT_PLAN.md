...
- `[x]` **Companion equipment**: DONE 2026-07-04 — `equipped_by` wearer (per-wearer slot exclusivity, party-membership validated), ally builds fold worn gear (`_worn_equipment_stats`), `EquipRequest.wearer` + wearer picker UI (player+party select, worn-by label). 743 green. Remaining `[manual]` live feel + the `[auto]` test item below.
- `[x]` `[auto]` **equip-lang API regression test**: DONE 2026-07-04 — `ApiEquipLangTest` (3 tests) in `tests/test_api.py` locks the localization contract: EN axis_labels/stakes have no Hangul, KO default preserves Korean, and the equip toggle works regardless of language. `make check` 763 green.
...
- `[x]` **3.5-flash adoption + caching track — CLOSED (2026-07-04 night)**: cache-prefix v2 + canon expansion shipped (prefix ≥4096 proven via a 4047-tok hit); measured hit rate 8% at 11-12k prompts → caching dropped as a cost lever. 3.5 ADOPTED on cloud (rev `00018-pj9`) + local; AGY QA PASS. Verdict + numbers `docs/plans/2026-07-04-gemini-2.5-vs-3.5-eval.md`. Remaining `[ ]` `[manual]`: live feel sign-off on 3.5 (prose/tone/length). Follow-ups CLOSED 2026-07-05: `[x]` prompt diet (11.5k → live 8.6k/turn, −25%, echo crowd-out bug fixed; 3.5 ≈ $1.0/loop now) + `[x]` key-beat model split (`GEMINI_MODEL_KEYBEAT`, live-verified, hybrid ≈ $0.5/loop) — ops menu is env-only (full-3.5 / hybrid / full-2.5), measured appendix in the eval doc. Hybrid prose feel = `[manual]` if ever enabled.
- `[x]` **Kiro lane runtime smoke**: DONE 2026-07-05 — 3 live iterations: root-cause profile fix (kiro agent JSON needs a `tools` array, not just `allowedTools`; upstreamed to plugin `3cec9a8`), residue-recovery protocol verified (`91ffcd3`), and a real `[auto]` item consumed end-to-end (`36ad55d` + external re-gate GREEN + ledger `verified`).
