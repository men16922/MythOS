...
Authority QA: `docs/test/neo_seoul_live_qa.md`. **Latest deploy = `mythos-api-00033-n4q` (2026-07-09, via `make deploy` = .env PROJECT_ID)** — full stack + mobile 대본 최우선 (#3) + #3b setup overflow + **Se-rin-in-variant-opening fix trio #4/#5/#6** (solo_opening · ally-writeback · loop-counter root fix; Neon+Vertex-verified). Deploy hygiene: use `make deploy` (pins .env project; ambient gcloud config once drifted → stray service in claude-study-501117, deleted). Completed 07-04/05 gameplay/companion/cloud-runtime/model-routing/Kiro work → `docs/COMPLETED_SUMMARY.md` M55-M56.

- `[ ]` **Se-rin auto-spawns in variant-loop COMBAT (owner-decided fix, OPEN)**: han/su_ah loops correctly have `_party.members=[]`, but a carried `trusted_se_rin` flag unlocks her ally entry (`combat_service.py:345` `unlock_flags ∩ flags`) so she fights anyway. Fix = **don't carry per-loop `met_`/`trusted_` companion flags into a new loop** (affection BONUS in `meta_progression.relationships` still carries → "faint familiarity" via NARRATIVE ECHOES; han loop → Han via variant meet-arc, Se-rin only if re-met at a mid node). Interactive (gameplay+feel), then redeploy (this + undeployed ping/legend `d175522`).
