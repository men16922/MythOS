...
- `[/]` **상태이상 시스템** — 설계 `docs/plans/2026-07-12-status-effects-design.md`. **slice 1 DONE (`b55b37b`)**: `status_effects` 채널 + 연소(턴 시작 1d4 DoT, 행동 전 사망 가능)/부식(장갑 -2) + `applies` 스킬 라이더 + 보드 배지 pill 행(💫/🔥/🧪) + KO/EN 로그 + 직렬화 왕복 tests. `[x]` **slice 2 DONE (`9ff6419`)**: 산성(방어-2, 하한1)/냉동(이동 불가·행동 가능, reachable=[] 어포던스)/감전(집중 회복+쿨다운 정지) 훅 + 배지 💧❄⚡ + KO/EN 로그. 콘텐츠 미배선이라 라이브 동작 불변. `[ ]` `[manual]` **slice 3: 부여 콘텐츠 매핑 밸런스 패스** (누가 뭘 거는가 — 설계 §Grants 초안 검토) 후 `[auto:codex]` scenario.json 배선. (apply 순간 전용 FX는 slice 3에서 콘텐츠와 함께.)
