...
- `[/]` **DS2 UNBLOCKED 2026-07-08** (owner sign-off, review packet `docs/plans/2026-07-08-ds2-sample-migration.md`: G1 size remap ✓ · G3 bake glow ✓ · G2 accept 6→8px ✓). **DS2-api + DS2-a..d DONE** (`make check` 953, emulator parity vs `.panel`): api = size remap (default size-2 = 16px) + glow baked + Surface forwards `...rest`/`id` + `as` prop; a = `StatusPanel`; b = aside (OperationMap ×2, Save/RunHistory); c = StoryPanel (board/roster/scene/narrative); d = onboarding `<section>` + dashboards (Progress/Tester). Remaining:
  - `[ ]` `[auto:claude]` **DS2-e … DS2-n** migrate the remaining ~12 `.panel` div sites, **one cluster per slice** (e = character/codex/skill panels; f = DevConsolePanel ×6; then the 2 deferred `<details className="panel">` chips via `as="details"` + bespoke non-`.panel` classes), behavior-preserving. Done = `make check` green + post-commit AGY live-QA not FAIL (objective refactor, auto-screened).
