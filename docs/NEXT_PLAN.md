...
- `[x]` **🚀 DEPLOYED LIVE (2026-06-29)** — `https://mythos-api-1004528040791.us-central1.run.app` (Cloud Run us-central1 + Neon PG18 + Vertex/GCS), end-to-end verified. EN default + admin key(`admin-43dc07f266c2`, uncapped) + tester cap 10. Keys `INVITE_KEY.md`(gitignored). **NEXT = human/비차단**: 결제 예산 알림 + Vertex 일일 쿼터 · 피드백 Google Form → 종료화면 · `?invite=` 링크 배포 → r/playtesters · `git push` · (선택) Neon TRUNCATE 테스트데이터 정리. 잔여 EN: K9 자연엔딩 스샷(사람 플레이), glass-library glossary, session `_outcome`. 런북 `docs/cloud/DEPLOY.md` §10.
- `[/]` **⓪ CBT 온보딩 UX DONE (2026-06-29, `daa5438`+`fce5874`, 미푸시, 608 green)**: (a) EN 경로선택지 라벨 영어화(`en.json` phrases); (b) **Option B 안정 신원** — `stablePlayerId()`가 초대키에서 결정적 player_id 파생 → 세이브가 키 따라 기기 이동(OAuth 불필요, 백엔드 upsert 활용); (c) **게임식 Save/Load 모달**(`SaveLoadModal.tsx` — 장면·캐릭터(display_name+루프별 archetype)·날짜·턴·STA/TEN·**썸네일**[큐레이티드 `/resources` 우선, 없으면 서명 에셋], 클라이언트 페이징 6/page·최근 60, `/save-slots` `limit`); (d) **초대키 게이트**(`InviteGate.tsx`+`GET /auth/verify-invite` — 부팅 프로브 401→키 입력 화면, 키 localStorage 저장·브라우저당 1회, 오류 fail-open). 배포 시 `MYTHOS_INVITE_KEYS`+테스터별 `?invite=` URL로 활성. **남은 EN: K6 전투·K9 종료화면 미검증(agent 가능).**
- `[/]` **① EN/KO 언어지원(영어 default)** = **EN 모드가 라이브 SPA에서 엔드투엔드 영어로 완성 (2026-06-28)**: API가 `RuntimeOptions.language`를 안 넣던 버그 수정(서사 EN) + S4 데이터 sweep(`localize.py` glossary 332+phrases를 모든 플레이어 엔드포인트에 적용 → 전투/스킬트리/캐릭터/상태/맵 영어) + 오프닝 이미지·헤더 수정. **EN 라이브-QA(Playwright) 2026-06-28: 잔존 한글 9건+상태문자열 클래스 전부 수정(604 green, 미커밋) → EN 전 화면 잔존=언어토글뿐.** 남은 것: default flip(KO live-QA 게이트) + 꼬리(glass-library glossary, session `_outcome`, **K6 전투·K9 종료화면 EN 미검증**). 상세 `docs/plans/2026-06-27-en-ko-localization.md` + `docs/test/neo_seoul_live_qa.md` §K. → **② GCP 클로즈베타**: **Vertex 어댑터 3종 코드 DONE + gate-verified** (`[/]`, 584 green) — narrative Gemini `JSONProvider`(`gemini_provider.py`, `MYTHOS_NARRATIVE_PROVIDER`, controlled gen + `stream`), Vertex Imagen `VisualProvider` + GCS `StorageAdapter`(`visual_service.py`, `MYTHOS_VISUAL_PROVIDER`/`MYTHOS_STORAGE_BACKEND`); fake-client 테스트(`test_gemini_provider`19+`test_vertex_visual`13), `[gemini]`/`[gcs]`/`[gcp]` extras, `.env.example`(SDK-native env명). **Vertex live-test DONE**(ADC, Gemini 3/3 + Imagen, thinking-budget 버그 수정) + **Cloud Run 컨테이너화·WS 로컬검증 DONE**(lean 287MB, `Dockerfile`/`make cloud-image`, storage env-driven). **이미지 write-path GCS 배선도 DONE**(`storage_adapter_for` + `RuntimeOptions.image_storage` env 기본값 → 단일 `MYTHOS_STORAGE_BACKEND`가 읽기·쓰기 일괄). 추가 §3 안전코드 DONE: 초대키 게이팅(`MYTHOS_INVITE_KEYS`)·테스터당 루프 캡(`MYTHOS_MAX_LOOPS_PER_PLAYER`)·Cloud Trace·배포 런북+비용(`DEPLOY.md`). **자율 코드 전부 완료(595 green).** 남은 것 = **전부 human/infra**: `[manual]` `gcloud run deploy` + DB(Neon/Cloud SQL) + GCS 버킷 + 결제 예산 알림 + env값 + 피드백 Google Form(CLOSED_BETA §5) → 최소기능 배포 → **r/playtesters 5~10명 모집** → 핵심루프 개선 → **r/aigamedev 결과·아키텍처 공유** → 공개베타. 전략 `docs/cloud/CLOSED_BETA_FEEDBACK_STRATEGY.md`(+`GCP_PLAN`/`CAREER_STRATEGY`). 제품 정체성=Vertex/Gemini AI 게임(로컬 LLM=dev only; `fully local` 프레이밍 폐기).
