...
- `[ ]` `[auto:claude]` skill/icon integrity invariant (**now UNBLOCKED — 11/11 icons present, 2026-06-20; not yet implemented**): every `combat.skills[].id` has `resources/neo-seoul/skills/<id>.png` + archetype base/learnable + `epiphany` references a real skill. Completion: **remove the `test_assets.py` skill-icon exclusion (~line 62)** + add the assertion, `make check` green.
