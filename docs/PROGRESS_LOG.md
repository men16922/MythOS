...
## 2026-06-28 — App.tsx decomposition slice 14: extract useViewModels hook (make check green)
- Status: Completed. Behavior-preserving extraction; `App.tsx` 696→673 lines.
- Changed: NEW `hooks/useViewModels.ts` (123 lines) owns the codex/dev/tab view-model cluster moved verbatim out of `App.tsx` — `codexLists` + `devConsoleData` (memoized `buildCodexLists`/`buildDevConsoleData` projections), `epiphanyNotice` (`buildEpiphanyNotice` of latest run, suppressed once `dismissedEpiphany` matches), `dismissEpiphany` (`useCallback` persisting the dismissal to `localStorage`), `tabNotices` (per-tab "new content" hints), and `handleTabClick` (set tab + lazy `loadCodex` for codex/character/skills/dev). All memo dep arrays + handler bodies identical to prior in-`App` form; source state passed via a props object (matches the sibling-hook convention). The `useInGameEpiphany` call was reordered **above** the new hook so its `showInGameNotice` (a `tabNotices` input) is defined first. Removed now-unused `useCallback` (React import) + the `buildCodexLists`/`buildDevConsoleData`/`buildEpiphanyNotice` imports from `App.tsx` (`useMemo` retained for `currentScenario`).
- Verified: `make check` green — ruff/eslint (0 warnings), mypy 126 files, tsc/vite build, **529 tests OK** (skipped 2). Bundle rebuilt (`static/app.js`). (Post-hoc IDE diagnostics about line ~674/`cinemaContext` were stale mid-edit LSP state — `tsc -b` ran clean inside `make check`.)
- Blockers: none.
- Next: App.tsx decomposition — next candidate = the onboarding `loadScenarios` setup `useEffect` + the remaining onboarding/connection state, or fold the logging helper (`logToConsole`) and small derived render flags.
...
- Changed: NEW `hooks/useSnapshotReceiver.ts` (110 lines) owns the snapshot-receive cluster moved verbatim out of `App.tsx` — `handleReceivedSnapshot` (apply a WS-confirmed snapshot: patch loop id/last snapshot, fold `choice_result.summary` into the last history item, `resolveImage` else pick the curated-anchor vs worker-off placeholder, `loadSlotsAndRuns`, BGM swap) and its internal `triggerCinematicEffects` (turn-0 Ken Burns + glitch entry motion, `prefersReducedMotion`-gated). All cross-cutting state/setters + sibling-hook helpers (`resolveImage`/`setImagePlaceholderText` from useSceneVisuals, `loadSlotsAndRuns` from useDataLoaders, `playBgm`/`playSfx` from useAudio) passed as props (matches the useCombatRest/useSessionControls props-object pattern). Hook call sits between `useDataLoaders` (provides `loadSlotsAndRuns`) and `useSessionLifecycle` (consumes `handleReceivedSnapshot`); the WS type switch `handleSocketMessage` still calls it (call-time closure). `triggerCinematicEffects` is internal (only `handleReceivedSnapshot` calls it) so it's not re-exported. Removed the now-unused `prefersReducedMotion` import from `App.tsx`.
- Verified: `make check` green — ruff/eslint, mypy 126 files, tsc/vite build, **529 tests OK** (skipped 2). Bundle rebuilt (`static/app.js`).
- Blockers: none.
- Next: App.tsx decomposition — next candidate = the WS frame handler (`handleSocketMessage`) + `beginStream`/`sendChoose` streaming cluster, or the remaining onboarding/scenario-load effects.
