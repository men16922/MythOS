...
- **UNDEPLOYED bundle (session #5, 2026-07-10) — `make check` 972, uncommitted-until-checkpoint.** On top of `00039-r27`: **(1) choice-latency decouple** — prod logs measured choices lagging p50 9.4s/max 30s + 4% failed (two 442s/478s hangs) because synchronous scene-image gen (p50 5.9s/max 14.8s) gated the snapshot inside `_commit_scene`; streaming now defers the image (`RuntimeOptions.defer_image`, trailing `visual` event → existing `visual_status` frame), shipping choices first. **(2) mobile touch fixes** — typewriter scroll jank (IntersectionObserver stick-to-bottom + instant scroll), combat portrait page-scroll (`touch-action:pan-x` on the board wrapper). **(3) density** — proportional Korean `--font-read` on narration/choices/objective (mono stays for HUD), STATUS drops raw loop UUID + `phase` enum on mobile. **(4) stat-name unify** — sheet/BoonOffer/rewards → intuitive 근력/지능/매력/민첩/관측 (flavor proper nouns kept). Emulator @390px verified; **NOT real-device / NOT deployed**. Next (owner): `! git push` + redeploy (image decouple is hot-path → live smoke) + real-device touch-scroll confirm.
