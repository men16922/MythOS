...
- **2026-07-20 CUSTOM ICON SET + SKILL LOADOUT EDITOR** — `GameIcon` inline-SVG set replaces text-presentation glyphs (⚔❖◎▣✚✦❒★◆ etc.) across route map/legend, choice chips, interstitial, save slots, tactical key, market, turn-order, epiphany banners; the CBT "⚔ → thin ✕" rendering report is resolved. Skill quick-slot setup moved off the bar into a dedicated 편성 modal (owner request; per-slot ⇄ picker removed and lock test re-pinned). `make check` 1161 green; rendered browser QA clean (map/legend + combat-sim loadout flow, console errors 0). Local commits — ride the next deploy.
...
- **Harness QA-reduction evidence gap — narrowed 2026-07-20**: release-bundle evidence is now 2/3 (07-19 local build + `00077-8g9` with 6/6 PASS, hashes 25/25). The 16-item checklist stays formally human-owned until a third distinct release repeats the contract and human minutes are measured.
