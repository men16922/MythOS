...
- **deploy gap (push DONE 2026-07-08)**: private-repo push is a safety hard-block for the agent → user pushes directly. **Origin is now caught up to `889f713`** (user pushed the S4 + P1.5 + Design-System + Landscape-Combat LC0-6 + DS2 batch). **Remaining gap is DEPLOY, not push**: Cloud Run rev `00025-856` predates all of it, so the live service lacks every 07-07/08 change — human redeploy + sign-off pending (any new agent commits, e.g. DS2-c, will re-open a small push delta).
