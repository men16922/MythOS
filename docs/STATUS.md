...
- **2026-07-11 rev `mythos-api-00047-hln`** (smoke health/root 200; `make check` **983** green): mobile UX — **tab-swipe** navigation (guarded against the combat board's pan-x wrapper + horizontal scrollers) + **inline story-history collapsed by default** (`<details>`; full 전체보기 overlay stays recent-20). Emulator/unit-verified; owner real-device pass pending.
