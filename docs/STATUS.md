...
- **2026-07-20 CUSTOM ICON SET (CBT feedback)** — `GameIcon` inline-SVG set replaces text-presentation glyphs (⚔❖◎▣✚✦❒★◆ etc.) across route map/legend, choice chips, interstitial, save slots, tactical key, market, turn-order, epiphany banners; the "⚔ → thin ✕" rendering report is resolved. `make check` 1160 green; rendered browser QA clean (compact+expanded map, 9 legend chips, console errors 0). Local commit — rides the next deploy.
