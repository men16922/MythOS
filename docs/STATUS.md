...
1. **Automatic AGY browser QA integration (WS-A..F DONE, default-on)**: the overnight runner is QA-aware by default (`OVERNIGHT_BROWSER_QA=auto`; `=0` kill-switch) — Stage-1 candidate filter (`browser-qa-filter.sh`) → AGY 2-stage decision (`browser-qa.sh`/`run-agy.sh`) → dedup ledger; post-commit hook after gate+critic + DONE-drain A/F sweep; PASS/SKIP continue, FAIL/NEEDS STOP+notify (no revert); `status.sh`/overnight-report expose QA evidence. Verified: 502 tests green + one real integrated run (`20260621-113313-drain`, Chrome DevTools, PASS_CANDIDATE, clean evidence). `make overnight*` stays the only operator flow; human sign-off authoritative. Plan `docs/plans/2026-06-21-overnight-auto-agy-qa.md` §20-21.
