...
- **DEPLOYED 2026-07-09 — latest rev `mythos-api-00031-pxk`** (env-preserving `--source .`, MODEL=gemini-3.5-flash, root=200; 00026-s2g→…→00030-f89→**00031-pxk**). Live: the full prior batch (DS0-DS3 · LC0-6 · S4 · P1.5 · opening/prose) **+ Se-rin-flash fix** (early `loop_meta` WS frame — owner-confirmed) **+ mobile UX** (header hidden mid-play + floating ⋯ · 16:9 banner · tab rename) **+ session #3 mobile "대본 최우선"** (CHARACTER chip dropped on coarse · ObjectiveStrip one-line collapsible · narration 46vh cap lifted · anchor-image grid blowout `minmax(0,1fr)` · tab-overflow fix) **+ #3b 390px audit fixes** (setup `<select>`/action-row overflow). **Remaining**: git ahead of origin (re-push, hard-block → user) + real-device touch/notch/URL-bar pass.
