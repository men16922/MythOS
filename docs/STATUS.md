...
- **DEPLOYED 2026-07-10 — latest rev `mythos-api-00034-8xj`** (project `project-ec7809f7`; deploy pins `.env` `PROJECT_ID` via `--project` — the ambient gcloud project drifted once and a deploy landed a stray service in `claude-study-501117`, now deleted). Live incl. all prior (mobile "대본 최우선" #3, #3b setup overflow, Se-rin-flash, DS/LC/opening, Se-rin fix trio #4/#5/#6, ping/legend `d175522`) **+ tae_o/han portrait-flash fix** (`6e5f13f`: dropped ambient common-noun detection keywords 사냥개/해커). **Remaining**: git ahead of origin (re-push, hard-block → user) + real-device mobile pass + live sign-off (tae_o flash real-device confirm).
- **Se-rin appearance — flag-carry RULED OUT, mid-loop reintroduction OPEN (2026-07-10).** Reproduced (`scratch/repro_serin.py`): a fresh loop-2+ variant carries **no met_/trusted_ contact flags + empty `_party`** even under a met+trusted meta → **no flag-carry into new-loop initial state** (the owner-decided "stop carrying flags" fix had no target). One han loop was confirmed clean. **But owner reports Se-rin STILL appearing mid-loop in a variant (screenshot: narration "…정세린이다", CHARACTER=물거미)** → a **separate vector**: the model reintroduces her during play (story-bible/echo leak or a legit Se-rin node), NOT initial-state seeding. NEXT: inspect the affected loop state (which turn her flag/presence appears, via model `world_delta` vs route node) before any fix — do not assume.
