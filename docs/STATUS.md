...
1. **§3 route-axis alignment + deployed owner verdict** (`docs/test/neo_seoul_live_qa.md`) — local same-character pair is banked and objectively diverged, but evidence-labeled choices did not add `evidence`; lock/fix label→perspective/axis alignment, then run the non-fallback dull/sensitive verdict on `00076-jhc`.
...
- **Harness QA-reduction evidence gap**: all six assertions pass local live calibration, but the evidence is still one local build rather than three distinct releases. The 16-item checklist remains formally human-owned until cross-release false-accept/human-minute measurements support changing 0/16.
...
- **Play-style consequence alignment**: the 2026-07-19 local pair proved different route state, but the evidence-first UI policy ended with no `evidence` tally and a `p_rescue` destination was presented as `단서 찾기`. Diagnose and regression-lock route label→applied perspective/axis semantics before treating the deployed §3 feel test as trustworthy; threshold 2 remains tunable.
