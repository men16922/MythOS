...
- **DEPLOYED 2026-07-10 — latest rev `mythos-api-00037-cj2`** (project `project-ec7809f7`; deploy pins `.env` `PROJECT_ID` via `--project`). Live incl. all prior (mobile, DS/LC/opening, Se-rin fix trio #4/#5/#6, ping/legend `d175522`, tae_o/han portrait-flash `6e5f13f`) **+ choice-gated stat voices w/ persona cast (`51b2264`) + companion appearance guardrail (`9bfce0d`) + Se-rin variant-opening combat-spawn root fix (`31bdceb`)**. **Remaining**: git ahead of origin (re-push, hard-block → user) + real-device mobile pass + live sign-off (tae_o flash + combat-spawn on a NEW variant loop + stat-voice gating/persona feel).
- **Se-rin combat-spawn in variant loops — ROOT FIXED 2026-07-10 (`31bdceb`).** Combat unlocks on `member OR unlock_flags∩flags`; the layer-0 opening anchor hardcodes `trusted_se_rin` and `effect` isn't variant-overridable, so han/kai loops inherited it (route_runtime applies mid-loop, bypassing the model-only validator clamp) → she spawned though absent from the opening. `_apply_anchor_variant` now strips se_rin contact flags from non-default variant anchors; a loop-start companion guardrail (`9bfce0d`) additionally denies any carried companion flag/party. Owner-live retest on a fresh variant loop pending. Any residual on an in-flight/legacy loop = new loop or 1-time DB cleanup.
