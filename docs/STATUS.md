...
- **DEPLOYED 2026-07-10 — latest rev `mythos-api-00041-kl5`** (Se-rin mid-loop casting guard, `dfede1e`, smoke 200). **Companion mid-loop reintroduction was a real OPEN vector, now fixed**: prod read of the reported loop (`loop_2a0ddb…`, su_ah variant, new loop, empty party, NO met_se_rin) proved Se-rin was **pure GM hallucination** (only flag `serin_trace_found` = model-invented, unauthored) — structured surfaces were all correct; the storyteller wrote her in. Fix = directive-layer only (past-ally echo reframed as absent + persistent COMPANION PRESENCE RULE with a meet-arc carve-out). Prompt-guard → confirm via fresh-su_ah live-QA.
- **DEPLOYED session #5 bundle 2026-07-10 (`mythos-api-00040-skm`→superseded by 00041-kl5, local-source build) — `make check` 972.** On top of `00039-r27`: **(1) choice-latency decouple** — prod logs measured choices lagging p50 9.4s/max 30s + 4% failed (two 442s/478s hangs) because synchronous scene-image gen (p50 5.9s/max 14.8s) gated the snapshot inside `_commit_scene`; streaming now defers the image (`RuntimeOptions.defer_image`, trailing `visual` event → existing `visual_status` frame), shipping choices first. **(2) mobile touch fixes** — typewriter scroll jank (IntersectionObserver stick-to-bottom + instant scroll), combat portrait page-scroll (`touch-action:pan-x` on the board wrapper). **(3) density** — proportional Korean `--font-read` on narration/choices/objective (mono stays for HUD), STATUS drops raw loop UUID + `phase` enum on mobile. **(4) stat-name unify** — sheet/BoonOffer/rewards → intuitive 근력/지능/매력/민첩/관측 (flavor proper nouns kept). Emulator @390px verified + live smoke 200; **NOT real-device**. Next (owner): `! git push` (repo sync — deploy was local-source, git still ahead of origin) + real-device touch-scroll confirm.
- **DEPLOYED 2026-07-10 — latest rev `mythos-api-00039-r27`** (project `project-ec7809f7`; deploy pins `.env` `PROJECT_ID` via `--project`). Live incl. all prior (mobile, DS/LC/opening, Se-rin fix trio, portrait-flash, stat voices, companion guardrail/combat-spawn/cutscene/callback fixes) **+ Se-rin bible present-gate (`2b5b50f`) + narrative clarity fix tracks 1-4** (patrol/heat labels `c58a2ac` · person-first identity + label unify + IX identity `ae21be0` · goal↔opening alignment `1d943f7` · **deterministic play-style consequence system `26cacc8`**). **Remaining**: git ahead of origin (re-push, hard-block → user) + real-device mobile pass + live sign-off.
