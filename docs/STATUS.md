...
1. **Owner live QA gate** (`docs/test/neo_seoul_live_qa.md`) — all agent lanes are drained; §1 A/B is DECIDED (rollback). Remaining owner-side: **§2 real-device RE-CHECK of the portrait hierarchy rework** (board dominance / pan feel / cut-in readability / 30dvh dock) · **§3 two-style playtest** (play-style consequence system balance). While playing on `00073`: confirm the first real-loop image, term-gloss chips (§8), and that idle errors no longer appear; hand loop ids to the agent for the golden eval bank.
...
- **Portrait combat REWORKED 2026-07-17 — real-device re-check pending**: the owner's first real-device pass found the board subordinate to the console, cinema cards overlapping, and the simulator forcing a boon pick → hierarchy rework shipped on `00073-zx2` (board height-fit 389px + board-first scroll, dock 30dvh, cinema cards 27/31vw, sim skips boons; `docs/plans/2026-07-17-portrait-combat-hierarchy.md`). Emulator-verified; QA guide §2 rewritten for the re-check (board dominance feel · pan feel · cut-in readability · 30dvh dock).
