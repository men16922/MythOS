...
2. **P1.5 mobile-first auto queue ALL LANDED 2026-07-08 overnight** (`f64e224..e1c8e7c`, `make check` **922** green) — Track M (M1 `100dvh` · M2 <600px font/44px-tap · M3 tap-tooltips ×3) + T6a-c concise-mode + T5a/T5b board affordance. Runner model pinned sonnet-5 (`overnight-settings.json`) after the Fable-quota stall; two doc-budget tidies kept the gate green. Remaining P1.5 `[auto:claude]` all `[blocked]` (T5c pending human iso-legibility judgment @390px; M3 remaining sites judged supplementary). **Not phone-tested — AGY @390px only; needs a real-device pass.** Design `docs/plans/2026-07-08-cbt-feedback3-clarity-plan.md`.
