...
- **DEPLOYED 2026-07-09 — latest rev `mythos-api-00033-n4q`** (project `project-ec7809f7`; use **`make deploy`** which pins `.env` `PROJECT_ID` — the ambient gcloud project drifted once and a deploy landed a stray service in `claude-study-501117`, now deleted). Live incl. all prior (mobile "대본 최우선" #3, #3b setup overflow, Se-rin-flash, DS/LC/opening) **+ the Se-rin-in-variant-opening fix trio: #4** solo_opening (bible `opening_variant` gate + carried-companion suppression) **· #5** ally-writeback (story-flag allies no longer promoted to permanent party) **· #6 TRUE ROOT** — Se-rin starting-party gate keys on `len(loops)` not `runs_completed` (an abandon-heavy tester kept runs_completed=0 → tutorial Se-rin re-seeded into every loop incl. variants; Neon-verified). **Remaining**: git ahead of origin (re-push, hard-block → user) + real-device pass + **OPEN (owner-decided): Se-rin still auto-spawns in variant-loop COMBAT via a carried `trusted_se_rin` flag — fix = don't carry per-loop met_/trusted_ companion flags across loops, affection bonus persists (PROGRESS_LOG #7)** + undeployed ping/legend (`d175522`).
