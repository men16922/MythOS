...
- **DEPLOYED 2026-07-09 — latest rev `mythos-api-00032-zrl`** (project `project-ec7809f7`; use **`make deploy`** which pins `.env` `PROJECT_ID` — the ambient gcloud project drifted once and a deploy landed a stray service in `claude-study-501117`, now deleted). Live incl. all prior (mobile "대본 최우선" #3, #3b setup overflow, Se-rin-flash, DS/LC/opening) **+ session #4: Se-rin no longer leaks into loop-2+ variant openings** (bible `opening_variant` gate + `solo_opening` suppression of carried met/party companions + solo directive; Vertex-verified turn 0-2 clean). **Remaining**: git ahead of origin (re-push, hard-block → user) + real-device pass + ally-writeback triage (root).
