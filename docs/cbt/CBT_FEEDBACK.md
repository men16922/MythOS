...
## #3 — 테스터 세션 (KO, "이용재", 오너 전달) — 2026-07-08

전 항목의 설계 대응: `docs/plans/2026-07-08-cbt-feedback3-clarity-plan.md` (트랙 T1~T6).
주제 한 줄: **게임이 의미를 감춘다** — 선택지·가치축·용어·스킬·보드가 초회 플레이어가 모르는
지식을 전제. + 신뢰 파괴 결함 2건.

- 🔴 **[T1·버그] 루프 중 이름 스왑**: 10번 장면에서 추격 선택지 클릭 → 이름 이용재→테스터로 변경.
  `create_player`는 display_name 업서트 확인됨 → 단순 rename 지연 아님. 유력 가설: 재접속 경로가
  localStorage/stable id의 **다른 플레이어 활성 루프로 갈아탐** (T2의 WS 드롭과 결합). /diagnose 대상.
- 🔴 **[T2·버그성] 선택지 3회 클릭해야 스트리밍 진행**: 진행 불능으로 오인 가능. 기지 리스크
  "WS 유휴 드롭(~45s)"의 실사용 증거로 판단 → keepalive+자동재접속+클릭 즉시 '전송 중' 피드백.
- 🔴 **[T3] 서사↔선택지 불일치**: 본문이 "왼쪽 지하철 폐노선 / 오른쪽 린위에 선착장, 선택해" 라고
  제시했는데 선택지는 뜬금없는 내용 → 프롬프트 계약(본문 갈림길=선택지 미러) + 목적지 1줄 설명.
- 🔴 **[T3·T4] 용어 난해**: "데이터 소각로로 향한다 — 거기가 뭐 하는 곳?" 류, 그들만의 언어 난무
  → route 목적지 설명 카피 전수 + 서사 내 용어→Codex 스니펫 링크.
- 🔴 **[T4] 가치축 불명**: 시민/관계·상호작용·탐색이 플레이에 뭘 바꾸는지 모름 (P1-A의 예상 변화
  문구로도 부족) → 축 칩 툴팁/1회 legend 오버레이 + 쉬운 말 카피 패스.
- 🔴 **[T4] 캐선창(아키타입 선택) 난해**: 설명이 lore-first → "이 선택이 바꾸는 것" 1줄 play-first 재작성.
- 🔴 **[T5] 전투 보드 이동 시점 불편 + 스킬 효과 불명**: #2의 D1 배지 이후에도 미해소 —
  보드 카메라/시점은 `[manual]` 설계 결정 후 슬라이스, 스킬 효과 라인은 문장형 쉬운 말 패스.
- 🔴 **[T6] UI 정보량 과다**: A3 점진 공개(루프1 턴0-2)로 부족 → 공개 창 연장 or 간결 모드 `[manual]` 결정.

## #2 — 오너 자가 플레이 (KO, 7루프, rev `00021~00024`) — 2026-07-05

전 항목의 설계 대응: `docs/plans/2026-07-05-cbt-onboarding-replay-density-plan.md` (트랙 B~F).

- 🔴 **다회차 동료 노출 실패**: 루프 7회에도 세린+카이만 만남 — 만남 아크가 시드 풀에서 안 뽑힘
  (지도당 2슬롯 / 풀 ~8, 특정 아크 확률 ~25%/판). → B1 보장 슬롯 + B2 Loop2+ 오프닝 변주 + B3 루프 모디파이어.
- 🔴 **노드 대사 반복/시간끌기**: 비분기 턴이 델타 없이 소모, 선택 반영 체감 약함. → C1 no-op 턴 가드
  (+ C2 페이싱 노브 `[manual]`).
- 🔴 **태오 뜬금 참전**: 최종 전투에 빌드업 0으로 합류 — 장면 연결성 약함. → C3 참전 예고 규칙.
- 🔴 **스킬 효과 불명**: 신호도약/글리치(이동기가 공격처럼 읽힘) · 엄호 노이즈(효과 비가시) ·
  과부하 일격(전맵 사거리 치트 체감, 밸런스 `[manual]`). → D1 카테고리 배지/기대효과 + D2 상태 칩 + D5.
- 🔴 **TACTICAL BOARD 협소 + 지형 오브젝트 무의미해 보임**: 엄폐 +3/+6은 실기능이나 비가독.
  → D3 보드 전폭 레이아웃(PARTY/ENEMY 하단 정렬) + D4 엄폐 가독화.
- 🟡 **SFX 부재**: 장면 전환에 소리 피드백 없음. → C4 결정론 SFX 큐.
- 🔴 **캐릭터 특성화 부족**: 동료 시그니처 스킬·보스 전용기 없음 (이미지는 codex/agy 파이프라인).
  → E 시그니처/보스 스킬 + F 스턴 기반(EMP 수류탄 미구현 발견과 공유).
