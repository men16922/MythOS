...
> **범용 개념 문서(bible).** 특정 repo 에 묶이지 않는다. 이 repo 적용(해석)은 → [`mythos/HARNESS.md`](mythos/HARNESS.md).
...
- **Measured Adoption**: 외부 도구·하네스 프롬프트를 처방 그대로 받지 않는다. 효과를 **측정해 조건부 정책으로 도출**하고 **제거 조건**을 단다. (vendor "X-FIRST/Y 회피" 프롬프트 → 실측 후 "대형·고빈도에서만, 권위는 원본" 처럼 한정.) 선택적 가속기는 **게이트화하지 않는다**(필수 빌드의존화 금지).
...
- 이 repo 적용: [`mythos/HARNESS.md`](mythos/HARNESS.md)
