...
### 3.4.1 Automatic live-QA (MythOS game specialization)
MythOS is a game, so the generic LOOP's lament — "feel-work can't be unattended-verified, so `[auto]` is thin" — only half holds: the **objective** part of browser behavior (boot / scene / choices / payload render, console/network) needs no human feel. With `OVERNIGHT_BROWSER_QA=auto` (default on), after the gate + critic a path-based candidate filter (`scripts/overnight/browser-qa-filter.sh`) decides whether a commit touches browser-observable behavior; if so AGY browser-tests it (Chrome DevTools → Playwright MCP) and `browser-qa.sh` records a dedup ledger. PASS/SKIP keep + continue; FAIL/NEEDS create `STOP` + notify (**never reverts**, no counter effect). A bounded A/F sweep also runs once at `DONE`.
This is an **evidence + stop-on-fail guard — not a backlog tag and not a deterministic gate**; a PASS is a human-review candidate, not a sign-off. Tagging payoff: objective UI/runtime **refactor / codemod / wiring** may be `[auto]`/`[auto:claude]` here (completion criterion adds "post-commit AGY live-QA not `FAIL_EVIDENCE`/`NEEDS_HUMAN`"), while subjective feel stays `[manual]`. Detail: [`VERIFICATION.md`](VERIFICATION.md) §4 · plan `bin/docs/plans/2026-06-21-overnight-auto-agy-qa.md`.
