...
현재 전술 전투 보드는 스냅샷당 1회 정적 리드로우만 한다. 유닛은 새 칸으로 순간이동하고, HP 바는 즉시 점프하며, 공격·피격·사망·스킬에 대한 시각 피드백이 전무하다. 이 작업은 전투를 **읽히는 사건의 연속**으로 만들기 위해 이동/공격/피격/사망/**스킬** 연출과 SFX 동기화를 도입한다. 체감 품질을 끌어올리되, 결정적(deterministic) E2E와 reduced-motion 경로를 깨지 않는 것을 제약으로 둔다.
...
- **Phase 1 — 기반 + 이동/데미지/힐 + 스킬 임팩트**: rAF 루프, `combatDiff`, 이동 트윈, 데미지/힐 플래시·숫자, HP 트윈, role 기반 스킬 임팩트(캐스트 큐+임팩트, 트레이서/lunge 포함), SFX 동기화. *백엔드 무변경, 체감 효과 최대.*
- **Phase 2 — 공격/사망 juice**: 기본 공격 lunge/슬래시 정밀화, 임팩트 스파크/파티클, 사망 디졸브, hit-stop, 선택적 스크린 셰이크, 스킬 캐스트 큐 고도화.
- **Phase 3 — (선택) 백엔드 전투 이벤트 로그**: `CombatService`가 순서 있는 이벤트(가해자→대상, crit/miss/multi-hit, 정확한 데미지) 방출 → 추론 제거, crit/miss/연타 분기 연출.
...
- **수동 QA**: `docs/play-checklist.md`에 스킬별 연출/데미지 숫자/이동 트윈/사망/실드/reduced-motion 항목 추가.
