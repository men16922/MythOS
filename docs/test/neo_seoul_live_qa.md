...
> (`make check` 620 green, 커밋 `ebc7905`~`ff0da98`, **미배포**). 배포(push+redeploy) 후 EN **새 루프**로
> 아래를 실플레이 확인. 발견 시 그 줄에 `[!]`+메모.

### 보스/전투 (최우선 — 사용자가 직접 겪은 멈춤 버그)
- [ ] **IX 보스전 실제 발동** — 작전지도 최종 boss 노드("Confront IX"/대면) 진입 시 실제로 IX 보스전투로 넘어가는가 (과거: 넘어가지 않고 멈춤).
- [ ] **보스전 체감** — IX(고체력 단일표적)+호위 드론+위험 지형에서 회복/엄호/일점사가 요구되나 / HP≤50% enrage 텔레그래프가 읽히나 / 파티로는 승산·솔로는 힘든가.
- [ ] **팬텀 전투 루프 없음** — 한 판 완주 중, 같은 접촉이 매턴 반복 발동되지 않는가 (다운그레이드/보스 우선 시 원접촉 해소 수정분).

### EN 한글 leak 재검 (이번 수정분 — EN 새 루프로 스팟 확인)
- [ ] **종료화면 EN** — 자연 엔딩까지 가서 ending 제목/서사가 영어로 납득되나 (엔딩 4종 제목+서사 글로서리 추가; K9 마지막과 동일).
- [ ] **전투 씬 텍스트 EN** — 전투 씬 제목("Engagement R…")·목표("Suppress the hostile signals…")·전투 로그가 영어인가 (K6와 묶음).
- [ ] **세이브 슬롯 라벨** — 전투 중 저장 시 "In combat …"로 영어인가 (과거 "전투 중 …").
- [ ] **코덱스/로어 패널** — Codex 로어 3항목(ARK 최적화/안드로이드의 꿈/관리망 정체) 제목·본문 영어인가.
- [ ] **스킬트리 에러 토스트** — 통찰 부족/미해금 스킬을 눌렀을 때 토스트가 영어인가 (과거 한글).
- [ ] **온보딩/이미지 상태문** — 접속·이어하기 상태줄, 이미지 플레이스홀더가 영어인가.
