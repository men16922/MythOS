...
최종 갱신: 2026-09-06. 이 문서는 자동 검사를 반복하지 않고, 직접 플레이가 필요한
...
- 상태이상이 같은 대상에 다시 걸리면 이제 **강도가 쌓입니다**(배지에 ×2·×3, 과열 피해·부식/용해
  약화가 그만큼 커짐; 동결·감전·조종은 지속만 늘어남). 너무 빨리 죽거나 반대로 체감이 없으면,
  어떤 상태이상이 몇 겹이었는지와 화면 크기(폰이면 ×N 표시가 읽히는지)를 적어 주세요. 아직 배포 전입니다.
