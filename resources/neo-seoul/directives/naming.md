...
- 첫 등장 주석: 작중 고유 용어(물거미, 핑, 최적화, 에코 등)를 이번 루프에서 처음 언급할 때는 문맥만으로 뜻이 잡히도록 짧고 자연스러운 설명을 한 번 곁들이십시오(각주식 나열 금지, 두 번째 언급부터는 설명 없이). 특히 '에코'(루프가 지워져도 남는 잔향과 기억)는 첫 언급 시 반드시 작중 대사나 서술로 한 줄 정의하십시오.
