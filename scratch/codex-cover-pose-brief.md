...
**[2026-07-12 4차] 이번 라운드는 `su-ah` 1장만.** 3차분(`cac23f9`)에서 player-noise는 **오너 승인 — 절대 덮어쓰지 말 것**.
su-ah 3차분은 무기(나이프)는 맞지만 **스타일이 회귀** — 오너 판정: "attempt-1 이미지 자체는 좋았고, 그 기준으로 무기만
바꾸면 됐다". 따라서:
- **`outputs/cover-pose-regen/su-ah-cover-attempt-1.png`를 1순위 생성 레퍼런스로 첨부**하고, 그 구도·포즈·코트의
  보라 회로 자수 질감·벨트 유틸리티 소품·조명·실드 형태를 **최대한 그대로 재현**할 것.
- 바꾸는 것은 단 하나: **데이터패드를 든 손 → guard와 동일한 나이프를 낮게 쥔 손**. 나머지 요소를 재해석하지 말 것.
- 셀프체크에 "attempt-1과 스타일/질감이 동일 계열인가?" 항목을 추가해 기록.

**[2026-07-12 3차] (완료)** 2장: `player-noise`(총기 제거 — PASS·오너 승인), `su-ah`(나이프 교체 — 무기 PASS·스타일 FAIL → 4차).
나머지 5장(se-rin/kai/lin-yue/tae-o/han)은 승격 확정, **덮어쓰지 말 것**. 아래 7장 기준 원문은 앵커 참고용으로 유지.
