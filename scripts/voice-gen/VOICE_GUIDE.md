...
- **모델**: `eleven_v3` 고정 (74개 언어, KO 포함). KO/EN은 **같은 voice_id**로 생성 — v3가 텍스트 언어를 따라감.
...
- 후보 풀: 계정 라이브러리(현재 KO 네이티브는 Jicheol·세종뿐) + **공유 라이브러리 검색**(language=ko,
  gender/age 필터)에서 보강. 영어 프리메이드의 KO 발화는 억양 편차가 크므로 KO 샘플 청취가 최종 판정.
