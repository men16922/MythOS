...
LANGUAGE_RULE = (
    "CRITICAL LANGUAGE RULE (필수 한국어 출력 규칙):\n"
    ...
    "3. 이 규칙은 절대적이며 최우선적으로 준수되어야 합니다."
)
    ...
    *,
    ...
    if isinstance(stats, dict):
        stat_descriptions = {
            "strength": {
                "name": "근력 (Strength)",
                "voice": "본능적이고 거칠며 물리적 파괴와 신체적 생존을 자극하는 육체의 목소리. 거칠고 직관적인 어투로 물리적 반응을 부추깁니다.",
            },
            "intelligence": {
                "name": "지능 (Intelligence)",
                "voice": "냉정하고 분석적이며 논리와 데이터, 시스템 최적화를 추구하는 연산의 목소리. 철저히 논리적이고 기계적인 어투로 이성적 계산을 제공합니다.",
            },
            "charisma": {
                "name": "매력 (Charisma)",
                "voice": "감정적이고 사교적이며 사람들의 심리와 가면 뒤의 진실을 읽는 감응의 목소리. 공감적이거나 기만적이고 매혹적인 어투로 타인의 감정을 조작하려 합니다.",
            },
            "agility": {
                "name": "민첩 (Agility)",
                "voice": "기민하고 신경질적이며 회피와 탈출, 위험 감지를 부추기는 반사의 목소리. 긴박하고 예민한 어투로 멈추지 말고 움직일 것을 다그칩니다.",
            },
            "perception": {
                "name": "관측 (Perception)",
                "voice": "예리하고 미세한 흔적과 보이지 않는 신호, 감춰진 디테일을 포착하는 감각의 목소리. 세밀하고 묘사적인 어투로 주변의 숨겨진 단서나 이질감을 짚어냅니다.",
            },
        }
        valid_stats = {
            k: int(v)
        ...
        if valid_stats:
            ...
            notes.append(
                ...
                f"다음 목소리를 자연스럽게 주입하십시오: \n"
                f" - {max_desc['name']}: {max_desc['voice']}\n"
                f"이 목소리는 괄호 표기법을 사용하여 표현해야 합니다. 예: (근력: 저 단단한 문을 어깨로 부수고 나아가라. 뼈가 울리겠지만 열릴 것이다.) 또는 (지능: 저 보안 패널의 깜빡임 주기는 3초다. 틈을 타 우회하면 감지되지 않는다.)"
            )
